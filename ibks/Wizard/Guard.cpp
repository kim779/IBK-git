...
int CGuard::Initial(CWnd* control)
{
	...
	// [KJS] 쓰레드 처리...  디버그시 쓰레드 처리하면 처리 안됨...
	//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
#ifndef _DEBUG
	...
	for (; true; )
	{
		...
		if (attr != 0xffffffff && !(attr & FILE_ATTRIBUTE_DIRECTORY))
		{
			if (GetString(AS_FILE, tmps))
				...
				string.Format(_T("이 위치에  \'%s\' 파일이 있습니다. 기존의 파일을(를) 바꾸시겠습니까?"), fileN.GetString());
			...
			case IDYES:
				if (!DeleteFile(fileN))
				{
					if (GetString(AS_USING, tmps))
						...
						string.Format(_T("\'%s\' 파일이 사용중입니다. 사용중인 파일을 닫고 다시 시도해 주십시오."), fileN.GetString());
