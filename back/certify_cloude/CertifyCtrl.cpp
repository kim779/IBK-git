...
//--------------------------------------------------------------------------------------------------------------------------------
			...
			if (rc) {
				m_slog.Format(" sk_if_cert_SignDataWithR 오류 rc = [%d]  errorCode=[%s]", rc, sk_if_GetLastErrorCode());
				FileLog(m_slog);
...
int CCertifyCtrl::Cloude_Full_sign(long pOutB, long pOutL)
{
	...
	if (rc) {
		...
		m_slog.Format(" 전자서명 오류 sk_if_cert_SignDataWithR rc = [%d]  errorCode=[%s]", rc, sk_if_GetLastErrorCode());
		FileLog(m_slog);
		CString emsg;
