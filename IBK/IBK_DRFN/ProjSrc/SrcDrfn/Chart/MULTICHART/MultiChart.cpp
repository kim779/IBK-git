...
extern "C" __declspec(dllexport) CWnd* CREATE_CHARTSCREENEX(int iTradingID, CWnd* pParent, HWND hPlatform, LPCSTR szOpenInfo)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());

	if(szOpenInfo==NULL)
		return CREATE_CHARTSCREEN(iTradingID, pParent, hPlatform);
	...
	InData.m_nID = iTradingID;
	InData.m_Rect = CRect(0,0,300,400);
	memcpy(InData.m_aOpenInfo, szOpenInfo, strlen(szOpenInfo));
