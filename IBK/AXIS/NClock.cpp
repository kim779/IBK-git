...
void CNClock::DrawClock(CDC* pDC, const CString& time)
{
...
void CNClock::DrawClockMini(CDC* pDC, const CString& time)
{
...
void CNClock::OnRButtonDown(UINT nFlags, CPoint point) 
{
	...
	const int cmd = menu.TrackPopupMenu(TPM_RETURNCMD|TPM_LEFTALIGN | TPM_LEFTBUTTON, lp.x, lp.y, this);
...
LRESULT CNClock::OnNcHitTest(CPoint point)  //vc2019 UINT->LRESULT
{
...
void CNClock::OnMainWndMoved()
{
	...
	const int cx = rc.left - m_rcMain.left;
	const int cy = rc.top - m_rcMain.top;
...
{
	const BOOL bOver = (CAL_LEFT<=point.x && point.x<=CAL_RIGHT) && (CAL_TOP<=point.y && point.y<=CAL_BOTTOM);
