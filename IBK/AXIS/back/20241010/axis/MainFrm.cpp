		...
		switch(pMsg->wParam)
		{
				...
				case 'F':
					...
					if (GetKeyState(VK_CONTROL) & 0x8000)
					{
						if (GetKeyState(VK_SHIFT) & 0x8000)
						{
							...
							m_slog.Format("tDept =[%s] m_Dept=[%s] keyin=[%s]", tDept, m_dept, stmp);
							WriteLog(m_slog);
				...
				/*	CString stmp, stitle;
					...
					ConclusionNotice(stmp, stitle);*/
						...
						//		if (m_pServerOrd)
						//		{
						//			m_pServerOrd->AddServerOrd("20240702172000\t주식자동주문서비스가신청되었습니다\t123", 300);
						//			m_pServerOrd->SetFocus();
						//		}
						//	}
						//	break;
						//}
...
// 		}
	...
	{
m_slog.Format("no 988 symbol  Alldata = [%s]", strback);
WriteUpLog("notice.ini", m_slog);
		ary.RemoveAll();
		return FALSE;
	}
...
void CMainFrame::DumpUpload()
{
	...
	m_slog.Format("DumpUpload  sALL =[%s]", sALL);
	WriteLog(m_slog);
	...
	if (!m_pUpload)
	{
		...
		{
			m_bUploadComplet = FALSE;
			m_slog.Format("DumpUpload  sALL == 1   path = [% s]  ", Axis::home + "\\user\\" + Axis::user + "\\Crashlog");
			WriteLog(m_slog);
...
void CMainFrame::initShared()
{
	...
	m_slog.Format("initShared noshared =[%s]", stmp);
	WriteLog(m_slog);
	...
	if (!m_hSharedLib)
	{
		WriteLog("cx_shared.dll LoadLibrary fail");
		return;
	}
	...
	if (axCreate == nullptr)
	{
		WriteLog("cx_shared.dll axcreate fail");
		return;
	}

	DWORD processID = GetCurrentProcessId();
	m_sHSharedkey.Format("%s%s%d", Axis::userID, "_HsharedCtrl_", processID);
	m_sMSharedkey.Format("%s%s%d", Axis::userID, "_MsharedCtrl_", processID);
