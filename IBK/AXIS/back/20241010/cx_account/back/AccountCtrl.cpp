...
////주문대리인 조회 관련 화면 분류
	...
	{
		m_bCheckAgetn = true;
		slog.Format("\r\n !!!!! -1로 파일에  cx_account [%s]  map0=[%s]    m_bCheckAgetn=[%d] m_bDLL=[%d]  !!!!", m_Param.options, mapname0, m_bCheckAgetn, m_bDLL);
		WriteLog(Variant(homeCC), slog);
		return;
	}
	...
	while (1)
	{
		...
		for (int ii = 0; ii < arr.GetSize(); ii++ )
		{
			...
			if (arr.GetAt(ii).Find(stmp) >= 0 && (strTemp == mapname0))
			{
				...
				slog.Format("\r\n  ---  map0=[%s]  strobjname=<%s>[%s]  idex=[%d] ---  m_bCheckAgetn=[%d] m_bDLL=[%d]  sresult=[%s] ii=[%d] size=[%d] bAccInMain=[%d]",
...
BOOL CAccountCtrl::Initialize(BOOL bDLL)
{
	...
	if (m_strDept != m_sAccnDept)
	{
		...
		if (nMapNo == 8401 || nMapNo == 1040 || nMapNo == 5000 || nMapNo == 2821 || nMapNo == 2831 || nMapNo == 2832 ||
			...
			nMapNo == 7141|| nMapNo == 1020)  //modi 7141 매매일지차트
		{
...
//FileLog(m_strRoot, m_slog);
		...
		QueryAccntCheck(m_pEdit->GetInputData());  //test
...
LRESULT CAccountCtrl::OnMessage(WPARAM wParam, LPARAM lParam)
{
	...
	case DLL_OUBx:
		...
		case TRKEY_CEHCKACC:
		{
			...
			m_slog.Format("[cx_account] @@@ dll_oubx sRet=[%s]", sRet);
	...
	case DLL_OUB:	// NOT USED. Use DLL_OUBx!!
		...
		{	
			...
			if (key == TRKEY_CEHCKACC)
			{
				...
				CString sRet;
				sRet.Format("%c", osacmq329->szAgntYN);
				m_slog.Format("[cx_account] !!! dll_oub sRet=[%s]", sRet);
				OutputDebugString(m_slog);
				if (sRet == "Y")   //test
				{
					ShowAcntCheckResult();
...
void CAccountCtrl::ParseAccntName(char *pBuf)
{
	...
	QueryAccntCheck(m_pEdit->GetInputData());  //test checkacc  , parseaccntname
