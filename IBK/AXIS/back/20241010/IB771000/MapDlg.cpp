...
// 				OutputDebugString(strTemp);
	...
	if (bAdd /*||*//* bAlert ||*//* bAlertx*/) 	//2012.01.19 KSJ AlertX 추가
	{
		...
		m_bADDING = TRUE;
m_slog.Format("[7710]  !!!!!!!!ADD[%x]!!!!!    GetObject [%d] size =[%d]", this, job.msg, m_jobList.GetSize());
OutputDebugString(m_slog);
...
{
	bshow = true;
	m_slog.Format("[7710] WorkObjec      -------------------------------------                       START<%d>[%x] m_bADDING=[%d]                              ", m_jobList.GetCount(), this, m_bADDING);
	OutputDebugString(m_slog);
}
...
if (bshow)
{
	m_slog.Format("[7710] WorkObjec         --------------------------------                           END<%d>[%x][%d]                                 ", m_jobList.GetCount(), this, m_bADDING);
	OutputDebugString(m_slog);
}
