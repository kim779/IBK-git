...
{
// 	TRACE(_T("in CImageTreeCtrl::CImageTreeCtrl\n"));
...
//=============================================================================
CImageTreeCtrl& CImageTreeCtrl::Initialize(BOOL bCheckBoxes /*= FALSE*/, 
								   BOOL bToolTip /*= FALSE*/)
//=============================================================================
{
// 	TRACE(_T("in CImageTreeCtrl::Initialize\n"));
...
//=============================================================================
// PreCreateWindow() is called when CImageTreeCtrl is used in a view.
//
BOOL CImageTreeCtrl::PreCreateWindow(CREATESTRUCT& cs) 
//=============================================================================
{
// 	TRACE(_T("in CImageTreeCtrl::PreCreateWindow\n"));
...
//=============================================================================
{
// 	TRACE(_T("in CImageTreeCtrl::PreSubclassWindow\n"));
...
// 			TRACE(_T("ESC seen during drag\n"));
		...
		{
			const BOOL bRight = pMsg->wParam == VK_RIGHT;
			...
			if (pXTCD && pXTCD->bEnabled && pXTCD->nChildren)
			{
				...
				const BOOL bOldExpanded = pXTCD->bExpanded;
				if (!bExpanded && bRight)
					bExpanded = TRUE;
				if (bExpanded && !bRight)
					bExpanded = FALSE;
		...
		{
			const BOOL bDown = pMsg->wParam == VK_DOWN;
...
//=============================================================================
void CImageTreeCtrl::OnCustomDraw(NMHDR* pNMHDR, LRESULT* pResult)
//=============================================================================
{
	...
	*pResult = CDRF_DODEFAULT;
	...
	//=========================================================================
	{
		...
		const COLORREF crAnchorText = m_crAnchorText;
		COLORREF crBackground = m_crWindow;
		const COLORREF crTextBackground = m_crWindow;
...
//=============================================================================
BOOL CImageTreeCtrl::SelectItem(HTREEITEM hItem)
//=============================================================================
{
...
//=============================================================================
BOOL CImageTreeCtrl::IsSelected(HTREEITEM hItem)
//=============================================================================
{
	BOOL rc = FALSE;
...
//=============================================================================
BOOL CImageTreeCtrl::IsEnabled(HTREEITEM hItem)
//=============================================================================
{
	BOOL rc = FALSE;
...
//=============================================================================
BOOL CImageTreeCtrl::IsExpanded(HTREEITEM hItem)
//=============================================================================
{
	BOOL rc = FALSE;
...
//=============================================================================
BOOL CImageTreeCtrl::IsSeparator(HTREEITEM hItem)			//+++1.6
//=============================================================================
{
	BOOL rc = FALSE;
...
//=============================================================================
BOOL CImageTreeCtrl::GetItemPath(HTREEITEM hItem, CStringArray& sa, CPtrArray& items)
//=============================================================================
{
	BOOL rc = FALSE;
...
//=============================================================================
int CImageTreeCtrl::GetDefaultTipWidth()
//=============================================================================
{
	int nWidth = 200;
	...
	if (m_nDefaultTipWidth == 0)
	{
		...
		if (pWnd && ::IsWindow(pWnd->m_hWnd))
		{
			...
			const int nWidthTree = (3 * rectTree.Width()) / 4;
			const int nWidthParent = rectParent.Width() / 2;
...
//=============================================================================
void CImageTreeCtrl::CreateToolTipsForTree()
//=============================================================================
{
	...
	const int nCount = m_nToolCount; //m_pToolTip->GetToolCount();
...
//               0 - no image list
int CImageTreeCtrl::GetNormalImageWidth(HTREEITEM hItem)
//=============================================================================
{
	int nWidth = 0;
...
//=============================================================================
BOOL CImageTreeCtrl::CreateCheckboxImages()
//=============================================================================
{
	CDC *pDC = GetDC();
	ASSERT(pDC);
	const BOOL rc = CreateCheckboxImageList(pDC, m_StateImage,
...
BOOL CImageTreeCtrl::CreateCheckboxImageList(CDC *pDC, 
				...
				nBmpHeight))
		{
					...
					nImages, 1))
			{
				...
				if (dcMem.CreateCompatibleDC(pDC))
				{
					...
					const int nImageWidth  = nBmpWidth - 2;		// allow 2 for border
					const int nImageHeight = nBmpHeight - 2;
					const int nImageLeft   = (nBmpWidth - nImageWidth) / 2;
					const int nImageTop    = (nBmpHeight - nImageHeight) / 2;
...
//=============================================================================
int CImageTreeCtrl::DrawItemText(CDC *pDC, 
							 HTREEITEM hItem, 
							 LPCTSTR lpszText,
							 COLORREF crText, 
...
// 		TRACE(_T("ERROR bad parameters\n"));
	...
	const CRect rectText(rect);
	pDC->FillSolidRect(&rectText, crBackground);

	CString str = lpszText;
	//TRACE(_T("CImageTreeCtrl::DrawItemText:  crText=%08X  crBkgnd=%08X  <%s> ++++++\n"), crText, crBackground, str);

	XHTMLTREEDATA *pXTCD = GetItemDataStruct(hItem);

	if (pXTCD && !str.IsEmpty())
	{
		const UINT uFormat = DT_VCENTER | DT_SINGLELINE | DT_LEFT | DT_NOPREFIX;
...
#ifdef XHTMLHTML
//=============================================================================
int CImageTreeCtrl::DrawItemTextHtml(CDC *pDC, 
								 HTREEITEM hItem, 
								 LPCTSTR lpszText,
								 COLORREF crText, 
								 COLORREF crTextBackground, 
...
// 	TRACE(_T("in CImageTreeCtrl::DrawItemTextHtml: <%s> crText=%06X  crBk=%06X\n"), strText, crText, crBackground);
...
//=============================================================================
int CImageTreeCtrl::DrawSeparator(CDC *pDC,						//+++1.6
							  HTREEITEM hItem, 
							  COLORREF crText, 
							  COLORREF crBackground, 
							  const CRect& rect)
...
//=============================================================================
{
// 	TRACE(_T("in CImageTreeCtrl::OnDestroy\n"));
...
//=============================================================================
void CImageTreeCtrl::DeleteMap()
//=============================================================================
{
	const BOOL bOldDestroyingTree = m_bDestroyingTree;
	m_bDestroyingTree = TRUE;
...
HTREEITEM CImageTreeCtrl::GetNextItem(HTREEITEM hItem)
//=============================================================================
{
...
// hItem        - The reference item
HTREEITEM CImageTreeCtrl::GetPrevItem(HTREEITEM hItem)
//=============================================================================
{
	HTREEITEM hItemPrev;
...
HTREEITEM CImageTreeCtrl::GetLastItem(HTREEITEM hItem)
//=============================================================================
{
...
HTREEITEM CImageTreeCtrl::FindItem(CString &str, 
...
BOOL CImageTreeCtrl::IsFindValid(HTREEITEM)
//=============================================================================
{
	return TRUE;
}
...
//=============================================================================
void CImageTreeCtrl::RedrawItem(HTREEITEM hItem)
//=============================================================================
{
	if (hItem)
	{
...
// 			TRACE(_T("cursor over state image\n"));
			...
			if ((nState & DISABLED_INDEX) == 0)		// is it enabled?
			{
				// a hot item can only be UNCHECKED, CHECKED, or CHECKED_TRISTATE
...
//=============================================================================
HTREEITEM CImageTreeCtrl::IsOverItem(LPPOINT lpPoint /*= NULL*/)
//=============================================================================
{
	CPoint point;
	if (lpPoint)
...
//=============================================================================
//DEL BOOL CImageTreeCtrl::IsOverAnchor(HTREEITEM hItem, CPoint point, CRect *pRect /*= NULL*/)
//DEL //=============================================================================
//DEL {
//DEL 	BOOL rc = FALSE;
//DEL 
//DEL 	CRect rect(0,0,0,0);
//DEL 
//DEL 	XHTMLTREEDATA *pXTCD = GetItemDataStruct(hItem);
//DEL 	
//DEL 	if (pXTCD && pXTCD->ds.bHasAnchor && pXTCD->bEnabled)
//DEL 	{
//DEL 		GetItemRect(hItem, &rect, TRUE);
//DEL 
//DEL 		// set rect to anchor boundaries
//DEL 		rect.left  = pXTCD->ds.rectAnchor.left;
//DEL 		rect.right = pXTCD->ds.rectAnchor.right;
//DEL 
...
//=============================================================================
HCURSOR CImageTreeCtrl::SetCursor(HCURSOR hCursor)
//=============================================================================
{
...
//=============================================================================
BOOL CImageTreeCtrl::IsLeftButtonUp()
//=============================================================================
{
	BOOL rc = FALSE;
...
void CImageTreeCtrl::OnTimer(UINT nIDEvent)
//=============================================================================
{
	CPoint point;
	::GetCursorPos(&point);
...
//=============================================================================
CImageTreeCtrl& CImageTreeCtrl::SetCheck(HTREEITEM hItem, BOOL fCheck /*= TRUE*/)
//=============================================================================
{
	ASSERT(hItem);
	...
	{
// 		TRACE(_T("in CImageTreeCtrl::SetCheck: %d  <%s>\n"), fCheck, GetItemText(hItem));
		
		XHTMLTREEDATA *pXTCD = GetItemDataStruct(hItem);

		if (pXTCD && pXTCD->bEnabled && !pXTCD->bSeparator)		//+++1.6
		{
			const BOOL bOldChecked = pXTCD->bChecked;
			pXTCD->bChecked = fCheck;

			const UINT nState = GetStateImage(hItem);
...
//=============================================================================
void CImageTreeCtrl::SetHotItem(HTREEITEM hItem, UINT nFlags)
//=============================================================================
{
	if (m_hHotItem && (m_hHotItem != hItem))
	{
...
//=============================================================================
LRESULT CImageTreeCtrl::SendRegisteredMessage(UINT nMessage, 
									   HTREEITEM hItem, 
									   LPARAM lParam /*= 0*/)
//=============================================================================
{
...
//=============================================================================
int CImageTreeCtrl::GetStateImage(HTREEITEM hItem)
//=============================================================================
{
	int nState = 0;
...
//=============================================================================
int CImageTreeCtrl::SetCheckParent(HTREEITEM hItem, int nCount)
//=============================================================================
{
// 	TRACE(_T("in CImageTreeCtrl::SetCheckParent:  nCount=%d  <%s>\n"), nCount, GetItemText(hItem));
	ASSERT(hItem);
...
// 		TRACE(_T("pXTCD->nChecked=%d  pXTCD->nChildren=%d \n"), pXTCD->nChecked, pXTCD->nChildren);
		...
		const BOOL bOldCheck = pXTCD->bChecked;
...
//=============================================================================
CImageTreeCtrl& CImageTreeCtrl::SetCheckChildren(HTREEITEM hItem, BOOL fCheck)
//=============================================================================
{
// 	TRACE(_T("in CImageTreeCtrl::SetCheckChildren\n"));
...
//=============================================================================
CImageTreeCtrl& CImageTreeCtrl::SetItemStateChildren(HTREEITEM hItem, BOOL fCheck)
//=============================================================================
{
	TRACE(_T("in CImageTreeCtrl::SetItemStateChildren\n"));
	...
	if (pXTCD && pXTCD->bEnabled)
	{
		...
		{
			const int nStateHot = nState & HOT_INDEX;			// save hot
			const int nStateDisabled = nState & DISABLED_INDEX;	// save disabled
			nState &= ~(HOT_INDEX | 
						DISABLED_INDEX);	// remove hot & disabled
...
//=============================================================================
BOOL CImageTreeCtrl::EnableItem(HTREEITEM hItem, BOOL bEnabled)
//=============================================================================
{
	BOOL rc = TRUE;
...
//=============================================================================
BOOL CImageTreeCtrl::EnableBranch(HTREEITEM hItem, BOOL bEnabled)
//=============================================================================
{
	BOOL rc = TRUE;
...
//=============================================================================
BOOL CImageTreeCtrl::GetCheck(HTREEITEM hItem)
//=============================================================================
{
	BOOL rc = FALSE;
...
//=============================================================================
HTREEITEM CImageTreeCtrl::GetFirstCheckedItem()
//=============================================================================
{
	if (m_bCheckBoxes)
	{
...
//=============================================================================
HTREEITEM CImageTreeCtrl::GetNextCheckedItem(HTREEITEM hItem)
//=============================================================================
{
	if (m_bCheckBoxes)
	{
...
//=============================================================================
HTREEITEM CImageTreeCtrl::GetPrevCheckedItem(HTREEITEM hItem)
//=============================================================================
{
	if (m_bCheckBoxes)
	{
...
//=============================================================================
BOOL CImageTreeCtrl::DeleteItem(HTREEITEM hItem)
//=============================================================================
{
	TRACE(_T("in CImageTreeCtrl::DeleteItem\n"));
	const BOOL bOldDestroyingTree = m_bDestroyingTree;
...
//=============================================================================
void CImageTreeCtrl::DeleteBranch(HTREEITEM hItem)
//=============================================================================
{
	if (hItem)
	{
...
//=============================================================================
CString CImageTreeCtrl::GetItemNote(HTREEITEM hItem, BOOL bStripHtml /*= FALSE*/)
//=============================================================================
{
	CString strNote = _T("");
...
//=============================================================================
int CImageTreeCtrl::GetItemNoteWidth(HTREEITEM hItem)
//=============================================================================
{
	int nWidth = 0;
...
//=============================================================================
CImageTreeCtrl& CImageTreeCtrl::SetItemNote(HTREEITEM hItem, 
									LPCTSTR lpszNote, 
									int nTipWidth /*= 0*/)
//=============================================================================
{
	...
	if (hItem && lpszNote)
	{
		...
		if (pXTCD)
		{
			...
			const size_t len = _tcslen(lpszNote);
...
//=============================================================================
HTREEITEM CImageTreeCtrl::InsertItem(LPTVINSERTSTRUCT lpInsertStruct, 
...
//=============================================================================
HTREEITEM CImageTreeCtrl::InsertItem(UINT nMask, 
								 LPCTSTR lpszItem, 
								 int nImage, 
								 int nSelectedImage, 
								 UINT nState, 
...
//=============================================================================
HTREEITEM CImageTreeCtrl::InsertItem(LPCTSTR lpszItem, 
...
//=============================================================================
HTREEITEM CImageTreeCtrl::InsertSeparator(HTREEITEM hItem)	//+++1.6
//=============================================================================
{
// 	TRACE(_T("in CImageTreeCtrl::InsertSeparator\n"));
...
//=============================================================================
int CImageTreeCtrl::IncrementChildren(HTREEITEM hItem, int n /*= 1*/)
//=============================================================================
{
	int nChildren = 0;
...
//=============================================================================
int CImageTreeCtrl::IncrementSeparators(HTREEITEM hItem, int n /*= 1*/)		//+++1.6
//=============================================================================
{
	int nSeparators = 0;
...
//=============================================================================
XHTMLTREEDATA * CImageTreeCtrl::GetItemDataStruct(HTREEITEM hItem)
//=============================================================================
{
	XHTMLTREEDATA *pXTCD = NULL;
...
void CImageTreeCtrl::OnDisplayTooltip(NMHDR * pNMHDR, LRESULT * pResult)
//=============================================================================
{
// 	TRACE(_T("in CImageTreeCtrl::OnDisplayTooltip\n"));
	*pResult = 0;
...
//=============================================================================
{
// 	TRACE(_T("in CImageTreeCtrl::OnToolTipText\n"));
...
BOOL CImageTreeCtrl::PreDisplayToolTip(BOOL bAlwaysRemoveHtml, CString& strToolTip)
//=============================================================================
{
	BOOL rc = FALSE;
...
// 		TRACE(_T("in PreDisplayToolTip:  mouse on item %X\n"), hItem);
		...
		if (pXTCD)
		{
			...
			const BOOL bNote = !strToolTip.IsEmpty();
...
// 				TRACE(_T("note or overflow\n"));
				...
				if (!lResult)
				{
					...
					const BOOL bStripHtml = bAlwaysRemoveHtml || !bNote;
...
//=============================================================================
void CImageTreeCtrl::OnSysColorChange() 
//=============================================================================
{
...
//=============================================================================
COLORREF CImageTreeCtrl::GetItemTextBkColor(HTREEITEM hItem)
//=============================================================================
{
	const COLORREF rc = 0;
...
//=============================================================================
COLORREF CImageTreeCtrl::GetItemTextColor(HTREEITEM hItem)
//=============================================================================
{
	const COLORREF rc = 0;
...
//=============================================================================
BOOL CImageTreeCtrl::SetItemText(HTREEITEM hItem, LPCTSTR lpszItem)		//+++1.6
//=============================================================================
{
...
//=============================================================================
COLORREF CImageTreeCtrl::SetItemTextBkColor(HTREEITEM hItem, COLORREF rgb)
//=============================================================================
{
	const COLORREF rc = 0;
...
//=============================================================================
COLORREF CImageTreeCtrl::SetItemTextColor(HTREEITEM hItem, COLORREF rgb)
//=============================================================================
{
	const COLORREF rc = 0;
...
//=============================================================================
COLORREF CImageTreeCtrl::SetBkColor(COLORREF rgb) 
//=============================================================================
{
	const COLORREF old = m_crWindow;
	if (rgb == COLOR_NONE)
...
//=============================================================================
COLORREF CImageTreeCtrl::SetTextColor(COLORREF rgb) 
//=============================================================================
{
	const COLORREF old = m_crWindowText;
	if (rgb == COLOR_NONE)
...
//=============================================================================
COLORREF CImageTreeCtrl::SetInsertMarkColor(COLORREF rgb)
//=============================================================================
{
	if (rgb == COLOR_NONE)
...
//=============================================================================
CImageTreeCtrl& CImageTreeCtrl::SetSeparatorColor(COLORREF rgb)	//+++1.6
//=============================================================================
{
	if (rgb == COLOR_NONE)
...
//=============================================================================
BOOL CImageTreeCtrl::EnableWindow(BOOL bEnable /*= TRUE*/)
//=============================================================================
{
	const BOOL rc = CTreeCtrl::EnableWindow(bEnable);
...
//=============================================================================
COLORREF CImageTreeCtrl::GetDisabledColor(COLORREF color)
//=============================================================================
{
...
//=============================================================================
CImageTreeCtrl& CImageTreeCtrl::SetLogfont(LOGFONT * pLogFont)
//=============================================================================
{
	ASSERT(pLogFont);
...
//=============================================================================
BOOL CImageTreeCtrl::SetItemBold(HTREEITEM hItem, BOOL bBold)
//=============================================================================
{
	const BOOL rc = FALSE;
...
//=============================================================================
BOOL CImageTreeCtrl::DeleteAllItems()
//=============================================================================
{
	...
	const BOOL bOldDestroyingTree = m_bDestroyingTree;
	m_bDestroyingTree = TRUE;
	const BOOL rc = CTreeCtrl::DeleteAllItems();
	DeleteMap();
	m_bDestroyingTree = bOldDestroyingTree;
	return rc;
}
...
//=============================================================================
void CImageTreeCtrl::CollapseAll()
//=============================================================================
{
...
//=============================================================================
void CImageTreeCtrl::ExpandBranch(HTREEITEM hItem)
//=============================================================================
{
	if (hItem && ItemHasChildren(hItem))
	{
...
//=============================================================================
void CImageTreeCtrl::CollapseBranch(HTREEITEM hItem)
//=============================================================================
{
	if (hItem && ItemHasChildren(hItem))
	{
...
//=============================================================================
BOOL CImageTreeCtrl::Expand(HTREEITEM hItem, UINT nCode)
//=============================================================================
{
	if (hItem && ItemHasChildren(hItem))
	{
...
// 	TRACE(_T("calling CTreeCtrl::Expand()\n"));
	const BOOL rc = CTreeCtrl::Expand(hItem, nCode);

	return rc;
}

//=============================================================================
BOOL CImageTreeCtrl::GetHasBeenExpanded(HTREEITEM hItem)
//=============================================================================
{
	BOOL rc = FALSE;
...
//=============================================================================
void CImageTreeCtrl::CheckAll(BOOL bCheck)
//=============================================================================
{
	if (m_bCheckBoxes)
	{
...
//=============================================================================
int CImageTreeCtrl::GetCheckedCount()
//=============================================================================
{
	int rc = 0;
...
//=============================================================================
int CImageTreeCtrl::GetChildrenCheckedCount(HTREEITEM hItem)
//=============================================================================
{
	int rc = 0;
...
//=============================================================================
int CImageTreeCtrl::GetChildrenDisabledCount(HTREEITEM hItem)
//=============================================================================
{
	int rc = 0;
...
//=============================================================================
int CImageTreeCtrl::GetChildrenCount(HTREEITEM hItem)
//=============================================================================
{
	int rc = 0;
...
//=============================================================================
int CImageTreeCtrl::GetSeparatorCount(HTREEITEM hItem)			//+++1.6
//=============================================================================
{
	int rc = 0;
...
//=============================================================================
CString CImageTreeCtrl::GetItemText(HTREEITEM hItem, BOOL bStripHtml /*= FALSE*/) const
//=============================================================================
{
	CString strText = _T("");
...
//=============================================================================
{
// 	TRACE(_T("in CImageTreeCtrl::OnLButtonDown\n"));
...
// 			TRACE(_T("sending WM_CLOSE to edit box\n"));
		...
		{
			const BOOL bExpanded = !pXTCD->bExpanded;
			Expand(hItem, bExpanded ? TVE_EXPAND : TVE_COLLAPSE);
			return;
		}
	}
...
//=============================================================================
void CImageTreeCtrl::OnRButtonDown(UINT nFlags, CPoint point) 
//=============================================================================
{
#ifdef XHTMLDRAGDROP
	EndDragScroll();
...
BOOL CImageTreeCtrl::OnClick(NMHDR* /*pNMHDR*/, LRESULT* pResult)
//=============================================================================
{
// 	TRACE(_T("in CImageTreeCtrl::OnClick\n"));
	...
	UINT flags = 0;
	HTREEITEM hItem = HitTest(point, &flags);
// 	TRACE(_T("in CImageTreeCtrl::OnClick:  hItem=%X\n"), hItem);
...
//=============================================================================
BOOL CImageTreeCtrl::OnDblclk(NMHDR* /*pNMHDR*/, LRESULT* pResult) 
//=============================================================================
{
// 	TRACE(_T("in CImageTreeCtrl::OnDblclk\n"));
	BOOL rc = FALSE;	// allow parent to handle
	...
	UINT flags = 0;
	HTREEITEM hItem = HitTest(point, &flags);
// 	TRACE(_T("in CImageTreeCtrl::OnDblclk:  hItem=%X\n"), hItem);
...
//=============================================================================
BOOL CImageTreeCtrl::OnSelchanged(NMHDR* pNMHDR, LRESULT* pResult) 
//=============================================================================
{
	BOOL rc = FALSE;	// allow parent to handle
...
// 	TRACE(_T("in CImageTreeCtrl::OnSelchanged:  <%s>\n"), strItem);
...
//=============================================================================
BOOL CImageTreeCtrl::OnSelchanging(NMHDR* pNMHDR, LRESULT* pResult) 
//=============================================================================
{
	BOOL rc = FALSE;	// allow parent to handle
	...
	CString strItem = GetItemText(hItem);
// 	TRACE(_T("in CImageTreeCtrl::OnSelchanging:  <%s>\n"), strItem);
...
//=============================================================================
void CImageTreeCtrl::OnSize(UINT nType, int cx, int cy) 
//=============================================================================
{
// 	TRACE(_T("in CImageTreeCtrl::OnSize\n"));
...
//=============================================================================
int CImageTreeCtrl::GetIndentLevel(HTREEITEM hItem)
//=============================================================================
{
	int nIndent = 0;
...
//=============================================================================
BOOL CImageTreeCtrl::ConvertBuffer(const BYTE * inbuf, 
...
//=============================================================================
BOOL CImageTreeCtrl::LoadXmlFromBuffer(const BYTE * pBuf, 
								   DWORD len,	// bytes
...
//=============================================================================
BOOL CImageTreeCtrl::LoadXmlFromResource(HINSTANCE hInstance, 
									 LPCTSTR lpszResId, 
									 LPCTSTR lpszResType,
									 ConvertAction eConvertAction)
//=============================================================================
...
//=============================================================================
BOOL CImageTreeCtrl::LoadXmlFromFile(LPCTSTR lpszFile, ConvertAction eConvertAction)
//=============================================================================
{
// 	TRACE(_T("in CImageTreeCtrl::LoadXmlFromFile: %s\n"), lpszFile);
...
//=============================================================================
HTREEITEM CImageTreeCtrl::InsertXmlItem(CXmlElement *pElement, HTREEITEM hParent)
//=============================================================================
{
// 	TRACE(_T("in CImageTreeCtrl::InsertXmlItem\n"));
...
//=============================================================================
BOOL CImageTreeCtrl::LoadXml(CXmlDocument& xml, 
...
//=============================================================================
CString CImageTreeCtrl::GetXmlText(HTREEITEM hItem, LPCTSTR lpszElem)
//=============================================================================
{
	ASSERT(lpszElem);
...
//=============================================================================
BOOL CImageTreeCtrl::SaveXml(HTREEITEM hItem, 
						 LPCTSTR lpszFile, 
...
//=============================================================================
LRESULT CImageTreeCtrl::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
//=============================================================================
{
	const LRESULT rc = CTreeCtrl::WindowProc(message, wParam, lParam);
	return rc;
}
...
//
BOOL CImageTreeCtrl::GetItemRect(HTREEITEM hItem, LPRECT lpRect, BOOL bTextOnly)
//=============================================================================
{
	const BOOL rc = CTreeCtrl::GetItemRect(hItem, lpRect, bTextOnly);

	if (bTextOnly)
	{
		const int width = GetNormalImageWidth(hItem);		// get width of normal image
...
//=============================================================================
BOOL CImageTreeCtrl::OnBeginlabeledit(NMHDR* /*pNMHDR*/, LRESULT* pResult) 
//=============================================================================
{
	*pResult = 0;
...
// 	TRACE(_T("in CImageTreeCtrl::OnBeginlabeledit: %X\n"), hItem);
...
//=============================================================================
BOOL CImageTreeCtrl::OnEndlabeledit(NMHDR* /*pNMHDR*/, LRESULT* pResult) 
//=============================================================================
{
// 	TRACE(_T("in CImageTreeCtrl::OnEndlabeledit\n"));
...
BOOL CImageTreeCtrl::OnCommand(WPARAM wParam, LPARAM lParam)
//=============================================================================
{
	CEdit *pEdit = GetEditControl();
...
//
void CImageTreeCtrl::AdjustEditRect()
//=============================================================================
{
	CEdit *pEdit = GetEditControl();
...
//=============================================================================
BOOL CImageTreeCtrl::IsCtrlDown()
//=============================================================================
{
	BOOL rc = FALSE;
...
//=============================================================================
BOOL CImageTreeCtrl::IsDragCopy()
//
...
//=============================================================================
HCURSOR CImageTreeCtrl::GetDragCursor()
//=============================================================================
{
	BOOL bCopyDrag = IsDragCopy();
...
//=============================================================================
void CImageTreeCtrl::SetDragCursor()
//=============================================================================
{
...
#ifdef _DEBUG
	CString strText = GetItemText(hItem);
	TRACE(_T("in CImageTreeCtrl::OnBegindrag: %s  bCopyDrag=%d\n"), strText, bCopyDrag);
#endif
...
//
BOOL CImageTreeCtrl::IsChildNodeOf(HTREEITEM hItem, HTREEITEM hitemSuspectedParent)
//=============================================================================
{
	do
	{
...
HTREEITEM CImageTreeCtrl::MoveItem(HTREEITEM hItem, 
...
HTREEITEM CImageTreeCtrl::MoveBranch(HTREEITEM hBranch, 
								 HTREEITEM hNewParent, 
								 HTREEITEM hAfter /*= TVI_LAST*/)
//=============================================================================
{
...
//=============================================================================
BOOL CImageTreeCtrl::StartMoveBranch(HTREEITEM hItem, 
								 HTREEITEM hNewParent, 
								 HTREEITEM hAfter /*= TVI_LAST*/)
//=============================================================================
{
...
// 	TRACE(_T("in CImageTreeCtrl::StartMoveBranch: %s %X:  new parent=%X  after=%X\n"), 
...
//=============================================================================
void CImageTreeCtrl::AutoScroll(HTREEITEM hItem)
//=============================================================================
{
...
//=============================================================================
void CImageTreeCtrl::EndDragScroll()
//=============================================================================
{
#ifdef XHTMLDRAGDROP

// 	TRACE(_T("in CImageTreeCtrl::EndDragScroll\n"));
