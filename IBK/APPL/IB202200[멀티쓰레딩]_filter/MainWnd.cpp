...
/////////////////////////////////////////////////////////////////////////////////////MAIN_RTS
static PFN_GET_TICK g_fnGetTick = nullptr;
extern "C" __declspec(dllexport)
void  RegisterGetTick(PFN_GET_TICK fn)
{
	g_fnGetTick = fn;
}

// DLL 내부에서 사용할 함수
BOOL TickApi_GetTick(const char* code, TickSnapshot* out)
{
	if (!g_fnGetTick || !code || !out)
		return FALSE;

	return g_fnGetTick(code, out);
}

extern BOOL TickApi_GetTick(const char* code, TickSnapshot* out);
/////////////////////////////////////////////////////////////////////////////////////MAIN_RTS
...
//		OutputDebugString(m_slog);
	...
	break;
	case MK_MAINOOPREGISTER:
	{
		/*ST_RT_SUBSCRIBE* pReq = new ST_RT_SUBSCRIBE{};
		pReq->hWnd = m_hWnd;
		pReq->reqType = RT_REQ_SUBSCRIBE;
		pReq->screenKey = m_pWnd->SendMessage(WM_USER, MAKEWPARAM(variantDLL, majorCC), NULL);
		pReq->mkType = m_pToolWnd->SendMessage(WM_MANAGE, MK_MARKET);
		
		CString stmp, token;
		int pos = 0;
		int cnt = 0;
		stmp.Format("%s", (char*)lParam);
		while ((token = stmp.Tokenize("\t", pos)) != _T(""))
		{
			strcpy_s(pReq->codes[cnt], (LPSTR)(LPCTSTR)token);
			cnt++;
		}
		pReq->count = cnt;
		m_pMainFrame->PostMessage(WM_USER, MMSG_SENDTR,  (LPARAM)pReq);*/
	}
	break;
	default:
		break;
...
void CMainWnd::GetMainFrameWnd()
{
	...
	m_pMainFrame->SendMessage(WM_USER, MMSG_SENDTR, 0);

	/*TickSnapshot snap{};
	if (TickApi_GetTick("A005930", &snap))
	{
		m_slog.Format("[RTS]   타입=[%s] 코드=[%s] 현재가=[%s] 거래량=[%s]  대비=[%s]  등락율=[%s]  틱=[%d][%d]",
			snap.RTStype, snap.code, snap.price, snap.volume, snap.diff, snap.rate, snap.ts_ms);
		OutputDebugString(m_slog);
	}*/
}
...
void CMainWnd::SendTRtoMain(CString trCode, char* datB, int datL, BYTE stat, int key)
{
	...
	m_pMainFrame->PostMessage(WM_USER, MMSG_SENDTR, (LPARAM)p);
