...
{
	_pApp = dynamic_cast<CIB202200App*>(AfxGetApp());
	SetIndex(nIndex);
	_classname = "CGridWnd";
	m_grid = nullptr;
	m_kind = 0;
...
void CGridWnd::sendTransactionTR(int update, int nStart, int nEnd)
{
	...
	int icnt{};
	CString tempStr;
	//const int itype = m_pToolWnd->SendMessage(WM_MANAGE, MK_MARKET);  //// 시장구분자   KRX :1  NXT :2  TOTAL : 3      NXT / KRX : 4 
	//tempStr.Format("%d\t", itype);
	...
	for_each(_vInters.begin(), _vInters.end(), [&](auto& pinters) {
		...
		if (!pinters.get()->code.IsEmpty())
		{
			...
			tempStr += "\t";
			//strcpy_s(pReq->codes[icnt], (LPSTR)(LPCTSTR)tempStr);
			icnt++;
		}
		CopyMemory(&sendB[sendL], tempB, strlen(tempB));
		sendL += strlen(tempB);
	...
	m_pMainWnd->SendMessage(WM_MANAGE, MK_MAINOOPREGISTER, (LPARAM)(LPSTR)(LPCTSTR)tempStr);
	m_slog.Format("[2022OOP] [%d][%s]", icnt, tempStr);
	OutputDebugString(m_slog);
...
//m_slog.Format("\r\n[interest][IB202200][%s][%d] 그룹명:[%s] 업로드", __FUNCTION__, gno, strGrouptName);
//OutputDebugString(m_slog);
