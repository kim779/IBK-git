...
#endif
#define USES_CONVERSION_NEW int _convert = 0; (_convert); UINT _acp = ATL::_AtlGetConversionACP() /*CP_THREAD_ACP*/; (_acp); LPCWSTR _lpw = NULL; (_lpw); LPCSTR _lpa = NULL; (_lpa)
#define USES_CONVERSION_NEW_CONST int _convert = 0; (_convert); const UINT _acp = ATL::_AtlGetConversionACP() /*CP_THREAD_ACP*/; (_acp); LPCWSTR _lpw = NULL; (_lpw); LPCSTR _lpa = NULL; (_lpa)
...
long CAccountCtrl::OnMessage(WPARAM wParam, LPARAM lParam)
{
	...
	case eventDLL:
		if (HIBYTE(HIWORD(wParam)) == evOnChange)	// get name
		{
			...
			return m_pParent->SendMessage(WM_USER + 1, MAKEWPARAM(100, HIWORD(wParam)), (LONG)(LPCTSTR)strData);
		}
		break;
...
/*
	CString data = (char*)m_pAccountCtrl->SendMessage(WM_USER + 1, 1, 0);
	CString acc, name, pass;
...
/*
bool CAccountCtrl::ConfirmPswd(CString pswd)
{
	return m_pAccountCtrl->SendMessage(WM_USER + 1, 2, (LPARAM)pswd.operator LPCTSTR()) ? true : false;
}
*/
...
void CAccountCtrl::ChangeFont(int ipoint, LPCTSTR lszname)
{
	CRect rc;
	GetClientRect(&rc);
	m_pAccountCtrl->MoveWindow(rc);
	m_pAccountCtrl->SendMessage(WM_USER, MAKEWPARAM(DLL_SETFONT, ipoint), (LPARAM)lszname);
}
