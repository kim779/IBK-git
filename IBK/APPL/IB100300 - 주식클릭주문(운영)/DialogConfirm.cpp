...
#define CLR_CONTENTSBG		GetIndexColor(181)
...
BOOL CDialogConfirm::Initialize()
{
	...
	m_pBtnOK->SetTextColor(RGB(71, 76, 80));
	...
	m_pBtnCancel->SetTextColor(RGB(71, 76, 80));
	...
	rcWindow.bottom = rcWindow.top + GetBackHeight() + 80 + 12;
	MoveWindow(&rcWindow);

	SetWindowText(_T("[1003] 주문::주식클릭주문"));
/*	
...
void CDialogConfirm::CreateTextWnd()
{
	...
	m_pData = std::make_unique< CTextWnd[] >(nCount);
	...
	//strJumnGubnText
	int iGubn = 0;
	iGubn = ((CMapWnd*)m_parent)->GetCondition();  //0현금 1신용 2대출

	strJumnGubnText = m_strOrderKind;

	if(iGubn == 0)
		strJumnGubnText = "현금" + strJumnGubnText;
	else if(iGubn == 1)
		strJumnGubnText = "신용" + strJumnGubnText;
	else if(iGubn == 2)
			strJumnGubnText = "대출" + strJumnGubnText;
	...
	case 1:			// 매수
		strTitle.Format("※%s주문확인창", strJumnGubnText);
	...
	case 2:			// 취소
		strTitle.Format("※%s확인창", strJumnGubnText);
	...
	if(iGubn == 1)
	{
		if( ((CMapWnd*)m_parent)->GetCreditType() == "03")
			strTitle += " (자기융자)";
		else if( ((CMapWnd*)m_parent)->GetCreditType() == "01")
			strTitle += " (유통융자)";	
	}
	...
	m_pTitle = new CTextWnd;
	m_pTitle->Create(strTitle, NULL, CRect(nX, nY, nWidth - 4, nY + ROW_HEIGHT), 
	...
	for (int i = 0; i < nCount; i++)
	{
		if (i == nTop)
			nY += 3;
		m_pHead[i].Create(arrHead[i], NULL, CRect(nX, nY, nX + 84, nY + ROW_HEIGHT),
							this, IDC_HEADBASE + i, NULL, clrHeadBack, CLR_FG, clrLine);
		m_pHead[i].SetFontFromParent(pFont);

		m_pData[i].Create(_T(""), NULL, CRect(nX + 83, nY, nWidth - GAP * 2, nY + ROW_HEIGHT),
	...
	m_pConfirm->Create(_T("주문처리하시겠습니까?"), NULL, CRect(nX, nY, nWidth - GAP * 2, nY + 18),
...
void CDialogConfirm::SetData(char cOrderKind, CString strAccount, CString strName, CString strJM, int iOrderCount, CString strOrderPrice, char cTradeKind, CString strOne, CString jmgbS)
{
	...
	m_strOrderCount.Format("%s  주", strTemp);
...
void CDialogConfirm::OnPaint() 
{
	...
	if(m_parent)
	{
		...
		dc.FillSolidRect(rectC, CLR_CONTENTSBG);
	...
	rcBack.SetRect(GAP, 22, rectC.Width(), 22 + nBackHeight);
