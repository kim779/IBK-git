...
int CMapWnd::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_slog.Format("[interest][IB202202][%s] m_bBookFileProcess=[%d]", __FUNCTION__, m_bBookFileProcess);
	OutputDebugString(m_slog);
...
void CMapWnd::uploadSequence()
{
	m_slog.Format("[interest][IB202202] -------------------------------------");
	OutputDebugString(m_slog);
	m_slog.Format("[interest][IB202202][%s]", __FUNCTION__);
	OutputDebugString(m_slog);
	...
	for(int i=0; i<count ; i++)
	{
		...
		m_slog.Format("[interest][IB202202]  ngrs=[%s] ogrs=[%s] gnam=[%s]", ngrs, ogrs, gnam);
		OutputDebugString(m_slog);
...
void CMapWnd::uploadEndGroup(int gno, int ipage)
{
	...
	m_slog.Format("[interest][IB202202][%s] gno=[%d] ogrs=[%s] gnam=[%s]", __FUNCTION__, gno, ogrs, gnam);
	OutputDebugString(m_slog);
...
BOOL CMapWnd::ChangeBookFile(int index, CString strFileO, CString strFileN, CStringArray& strarr) //bookmark fix
{
	...
	{	// 원래 그룹에 해당하는 북마크 파일, 새로운 그룹에 해당하는 북마크 파일 둘다 없다
		m_slog.Format("\r\n [interest][IB202202] 그룹[%d] | 북마크파일이 둘다없다 | O=[%s] N=[%s] ",
	...
	// 새로운 그룹 인덱스로 북마크 파일 생성, 기존 원본 북마크는 지운다
		...
		m_slog.Format("\r\n [interest][IB202202] 그룹[%d] | 북마크파일 원본O신규X 원본그룹북마크를 신규그룹북마크로 tmp카피 후 원본그룹 삭제| O=[%s]  N=[%s] ",
			index, strFileO.Right(12), strFileN.Right(12));
		OutputDebugString(m_slog);
		strarr.Add(strFileO);
		return TRUE;
	...
	{ //현재 이 그룹(index)이 신규가 되었지만 원래 북마크가 없으므로 복사할것도 지울것도 없다
	 //위의 조건를 갖는 그룹과 서로 바뀌는 그룹이다. 신규가 있는건 이것과 바뀌는 그룹의 원본이 있기 때문이다
		m_slog.Format("\r\n [interest][IB202202] 그룹[%d] | 북마크파일 원본X신규O 원래없으므로 카피도 삭제도 없다 | O=[%s] N=[%s] ",
			index, strFileO.Right(12), strFileN.Right(12));
		OutputDebugString(m_slog);
		return FALSE;
	}
	...
	{ //둘다 북마크가 있는 그룹을 서로 바꾼다. 이는 그룹 루프를 돌면서 두번 조건에 걸린다. 
	  //그때마다 원본파일을 tmp로 복사해두고 지우는 목록에 넣어둔다.
		m_slog.Format("\r\n [interest][IB202202] 그룹[%d] 북마크파일 원본O신규O 신규그룹북마크로 tmp카피 | O=[%s]  N=[%s] ",
			index, strFileO.Right(12), strFileN.Right(12));
		OutputDebugString(m_slog);
		CopyFile(strFileO, strFileN + "tmp", false);
		strarr.Add(strFileO);
...
m_slog.Format("\r\n [interest][IB202202] i=[%d] oldnoS=[%s]  gnoS=[%s]", i, oldnoS, gnoS);
OutputDebugString(m_slog);
			...
			{
				m_slog.Format("\r\n [interest][IB202202] DeleteFile=[%s]", strArrDelFile.GetAt(ii));
				OutputDebugString(m_slog);
