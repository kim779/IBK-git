...
void CSiseWnd::SetData(SiseData *psd)
{
	...
	pc = (CGridCell*)GetCell(0, 1);
	pc->SetTextClr( col );
	pc->SetText( DiffStr(psd->udgb, psd->diff,0) );
	//pc->SetData(psd->diff);
	RedrawCell(0, 1);
	...
	CString sData = (LPCSTR)pc->GetData();



	if (pc->GetData() && (LPCSTR)pc->GetData() != Int2Str(psd->siga))
	{
		pc->SetTextClr(GetColor(psd->gjga, psd->siga));
		pc->SetText(Int2Str(psd->siga, 0));
		//pc->SetData(psd->siga);
		RedrawCell(1, 1);
	}
	// 고가변경
	...
	if (pc->GetData() && (LPCSTR)pc->GetData() != Int2Str(psd->koga))
	{
		pc = (CGridCell *)GetCell(1, 3);
		pc->SetTextClr( GetColor(psd->gjga, psd->koga) );
		pc->SetText( Int2Str(psd->koga,0) );
		//pc->SetData(psd->koga);
		RedrawCell(1, 3);
	}
	...
	if (pc->GetData() && (LPCSTR)pc->GetData() != Int2Str(psd->jega))
	{
		pc = (CGridCell *)GetCell(2, 1);
		pc->SetTextClr( GetColor(psd->gjga, psd->jega) );
		pc->SetText( Int2Str(psd->jega,0) );
		//pc->SetData(psd->jega);
		RedrawCell(2, 1);
	}
	...
	if (pc->GetData() && (LPCSTR)pc->GetData() != Int2Str(psd->gvol))
	{
	...
	if (pc->GetData() && (LPCSTR)pc->GetData() != Int2Str(psd->mgjv))
	{
...
void CSiseWnd::SetRealData( RealData *rp )
{
	...
	{
		SiseData sd;
		sd.curr = (int)fabs(Str2Int( rp->find(23)->second ));
		sd.rate = (int)fabs(Str2Int( rp->find(33)->second ));
		sd.gvol = Str2Int( rp->find(27)->second );
		sd.siga = (int)fabs(Str2Int( rp->find(29)->second ));
		sd.koga = (int)fabs(Str2Int( rp->find(30)->second ));
		sd.jega = (int)fabs(Str2Int( rp->find(31)->second ));
		sd.mgjv = Str2Int( rp->find(201)->second );

		CString &diff = rp->find(24)->second.ToCStr();
		sd.udgb = diff.GetAt(0);
		sd.diff = Str2Int( diff.Mid(1) );
...
void CSiseWnd::SetRealData( DWORD* data )
{
	if (data[34])	// 체결시간이 있으면~
	{
		...
		sd.curr = (int)fabs(atoi( strCurr ));

		CString strRate;
		strRate.Format("%s",(char*)data[33]);
		sd.rate = (int)fabs(atoi( strRate ));
		...
		sd.siga = (int)fabs(atoi( strSiga ));

		CString strKoga;
		strKoga.Format("%s",(char*)data[30]);
		sd.koga = (int)fabs(atoi( strKoga ));

		CString strJega;
		strJega.Format("%s",(char*)data[31]);
		sd.jega = (int)fabs(atoi( strJega ));
