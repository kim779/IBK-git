...
//		TRACE("매입 %f 신용 %f 평가 %f 손익 %F\n", dMaip, symt, dprice, dSonik);
	...
	const double cur_tax = RoundDown2(fabs(atof(m_curr)) * m_dTax);
	...
	//	else dbep = fabs(atof(m_pmaip))*(1+m_dMass) + fabs(atof(m_curr))*(m_dMass+TAX_RATE);
		...
		{			
			...
			m_biyo.Format("%d",(int)((int)RoundDown2(fabs(forby) * m_dTax)+RoundDown2(fabs(forby) * m_dMass)));
		}	
		else
		{
...
//	CString strTemp;
// 	strTemp.Format("[KSJ][ParsingRemainData222] 융자금포함[%d] 신용[%s] 종목[%s](%s) dFee(%d) 수량(%d) 단가(%f) 매입금액(%.f) 원매입금액(%.f) 평가금액(%f) 체결여부(%d) 수익률(%f) 신용금액(%f) 현재가(%s) TAX(%f) 수수료(%f)",
// 		m_dCalcType, m_creditS, m_name, m_code, m_dFee, atoi(m_remain), atof(m_pmaip), atof(m_maip), atof(m_omaip), atof(m_rprice), m_bchg ? 1:0, atof(m_psuik), atof(m_symt), m_curr, m_dTax, m_dMass);
// 	OutputDebugString(strTemp);
// 	testSaveFile(strTemp);
