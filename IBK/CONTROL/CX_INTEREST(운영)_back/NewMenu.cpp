...
FktIsThemeActive pIsThemeActive = nullptr;
FktSetWindowTheme pSetWindowTheme = nullptr;
...
#ifdef _DEBUG
static void ShowLastError(LPCTSTR pErrorTitle = nullptr)
{
	if (pErrorTitle == nullptr)
	{
	...
	if (error)
	{
		...
		{
			// Display the string.
			MessageBox(nullptr, (LPCTSTR)lpMsgBuf, pErrorTitle, MB_OK | MB_ICONINFORMATION);
			// Free the buffer.
			LocalFree(lpMsgBuf);
		}
		else
		{
			...
			// Display the string.
			MessageBox(nullptr, temp, pErrorTitle, MB_OK | MB_ICONINFORMATION);
		}
	}
}
#else
...
UINT GetSafeTimerID(HWND hWnd, const UINT uiElapse)
{
	...
	{ // Try to start with TimerID 31, normaly is used 1, 10, or 100
		for (UINT nTimerId = 31; nTimerId < 100 && nNewTimerId == 0; nTimerId++)
...
public:
	...
	CNewLoadLib(LPCTSTR pName, LPCSTR pProgName)
	{
		...
		m_pProg = m_hModule ? (void *)GetProcAddress(m_hModule, pProgName) : nullptr;
	}
	...
	while (nCount)
	{
		...
		if (pTemp)
		{
			...
			if (*(pTemp + 1) == _T('&'))
			{
				...
				ExtTextOut(hDC, pos.x, pos.y, ETO_CLIPPED, lpRect, pBuffer, nTempCount, nullptr);
				nCount -= nTempCount + 1;
				pBuffer = pTemp + 2;
			}
			else
			{
				...
				if (!(uFormat & DT_HIDEPREFIX))
				{
					...
					case 0:
						...
						ExtTextOut(hDC, pos.x, nTop, ETO_OPAQUE, &rc, _T(""), 0, nullptr);
						break;
					...
					case 900:
						...
						ExtTextOut(hDC, nTop, pos.y, ETO_OPAQUE, &rc, _T(""), 0, nullptr);
						break;
		...
		else
		{
			...
			ExtTextOut(hDC, pos.x, pos.y, ETO_CLIPPED, lpRect, pBuffer, nCount, nullptr);
			break;
		}
	}
	// restore old point
...
BOOL GUILIBDLLEXPORT TrackPopupMenuSpecial(CMenu *pMenu, UINT uFlags, CRect rcTBItem, CWnd *pWndCommand, BOOL bOnSide)
{
	...
	if (bOnSide)
	{
		if (CenterPoint.x < rcTBItem.right)
		{
			...
			if (rcTBItem.left <= rect.left)
			{
				rcTBItem.left = rect.left + 1;
			}
			if (rcTBItem.top < rect.top)
			{
				rcTBItem.top = rect.top;
			}
			if (IsMirroredWnd(pWndCommand->GetSafeHwnd()))
			{
				return pMenu->TrackPopupMenu(TPM_LEFTBUTTON | uFlags, rcTBItem.left - 1, rcTBItem.top + 1, pWndCommand, nullptr);
			}
			else
			{
				return pMenu->TrackPopupMenu(TPM_LEFTBUTTON | TPM_RIGHTALIGN | uFlags, rcTBItem.left - 1, rcTBItem.top + 1, pWndCommand, nullptr);
			}
		}
		else
		{
			if (rcTBItem.right <= rect.left)
			{
				rcTBItem.right = rect.left + 1;
			}
			if (rcTBItem.top < rect.top)
			{
				rcTBItem.top = rect.top;
			}
			return pMenu->TrackPopupMenu(TPM_LEFTBUTTON | uFlags, rcTBItem.right - 1, rcTBItem.top + 1, pWndCommand, nullptr);
		}
	}
	else
	{
		...
		if (bMenuAbove)
		{
			...
			if (rcTBItem.left < rect.left)
			{
				rcTBItem.left = rect.left;
			}
			if (rcTBItem.top > rect.bottom)
			{
				rcTBItem.top = rect.bottom;
			}
			if (IsMirroredWnd(pWndCommand->GetSafeHwnd()))
			{
		...
		else
		{
			...
			if (rcTBItem.bottom < rect.top)
			{
				rcTBItem.bottom = rect.top;
			}
			if (IsMirroredWnd(pWndCommand->GetSafeHwnd()))
			{
...
CBrush *GetMenuBarBrush()
{
	...
	static CNewBrush *lastBrush = nullptr;
...
void UpdateMenuBarColor(HMENU hMenu)
{
	...
	if (IsMenu(hMenu) && DYNAMIC_DOWNCAST(CNewMenu, CMenu::FromHandlePermanent(hMenu)) != nullptr)
	{
	...
	if (pWinApp)
	{
		...
		if (pManager)
		{
			...
			while (pos)
			{
				...
				{
					// Change color only for CNewMenu and derived classes
					if (DYNAMIC_DOWNCAST(CNewMenu, CMenu::FromHandlePermanent(pMultiTemplate->m_hMenuShared)) != nullptr)
					{
				...
				// Change color only for CNewMenu and derived classes
				// need for correct menubar color
				if(DYNAMIC_DOWNCAST(CNewMenu,CMenu::FromHandlePermanent(pTemplate->m_hMenuInPlace))!=nullptr)
				{
				// menu & accelerator resources for in-place container
				SetMenuInfo(pTemplate->m_hMenuInPlace,&menuInfo);
				}
				if(DYNAMIC_DOWNCAST(CNewMenu,CMenu::FromHandlePermanent(pTemplate->m_hMenuEmbedding))!=nullptr)
				{
				// menu & accelerator resource for server editing embedding
				SetMenuInfo(pTemplate->m_hMenuEmbedding,&menuInfo);
				}
				if(DYNAMIC_DOWNCAST(CNewMenu,CMenu::FromHandlePermanent(pTemplate->m_hMenuInPlaceServer))!=nullptr)
				{
	...
	if (nItemIndex != UINT(-1) && GetMenuItemRect(pWnd->m_hWnd, pMenu->m_hMenu, nItemIndex, &itemRect))
	{
		...
		if (pMenu->GetMenuItemInfo(nItemIndex, &menuInfo, TRUE))
		{
			...
			if (menuInfo.fType & MF_OWNERDRAW)
			{
				...
				SendMessage(pWnd->GetSafeHwnd(), WM_DRAWITEM, 0, (LPARAM)&drwItem);
				SetWindowOrgEx(dc.m_hDC, windowOrg.x, windowOrg.y, nullptr);
...
public:
	...
	public:
		...
		//, m_hRightShade(nullptr),m_hBottomShade(nullptr),m_TimerID(0)
		{
			// Safe actual menu
...
CString CNewMenuItemData::GetString(HACCEL hAccel)
{
	...
	int nEntries = ::CopyAcceleratorTable(hAccel, nullptr, 0);
	if (nEntries)
	{
		...
		if (::CopyAcceleratorTable(hAccel, pAccel, nEntries))
		{
			...
			for (int n = 0; n < nEntries; n++)
			{
				...
				if (pAccel[n].fVirt & FALT)
					sAccel += _T("Alt+");
				if (pAccel[n].fVirt & FCONTROL)
					sAccel += _T("Ctrl+");
				if (pAccel[n].fVirt & FSHIFT)
					sAccel += _T("Shift+");
...
DWORD CNewMenu::m_dwLastActiveItem = 0;
	...
	else
	{
		if (IsMenuThemeActive())
		{
			...
			// if (pWnd)
			{
				...
				HANDLE hTheme = localOpenThemeData(nullptr, L"Window");
				if (hTheme)
				{
...
{
	auto dwAktTicks = GetTickCount64();
	static ULONGLONG dwLastTicks = 0;
...
void CNewMenu::DrawMenuTitle(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
	...
	if (hWnd == nullptr || !GetClientRect(hWnd, rcClipBox))
	{
	...
	pDC->TextOut(TextPoint.x, TextPoint.y, pMenuData->GetString(m_bDrawAccelerators ? m_hAccelToDraw : nullptr));
	...
	if (pMenuData->m_nMenuIconOffset != (-1) && pMenuData->m_pMenuIcon)
	{
		...
		pDC->DrawState(ptImage, iconSize - CSize(4, 4), hDrawIcon2, DSS_NORMAL, (HBRUSH) nullptr);
...
void CNewMenu::DrawItem_WinXP(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
	...
	else
	{
		...
		CString strText = pMenuData->GetString(m_bDrawAccelerators ? m_hAccelToDraw : nullptr);
		...
		if (!strText.IsEmpty())
		{
			...
			{
				// rotate font 90?        logFontMenu.lfOrientation = -900;
...
void CNewMenu::DrawItem_Icy(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
	...
	else
	{
		...
		CString strText;
		strText = pMenuData->GetString(m_bDrawAccelerators ? m_hAccelToDraw : nullptr);
		if (!strText.IsEmpty())
		{
...
void CNewMenu::MeasureItem_OldStyle(LPMEASUREITEMSTRUCT lpMIS, BOOL bIsMenuBar)
{
	...
	if (state & MF_SEPARATOR)
	{
		if (pMenuData->m_nTitleFlags & MFT_TITLE)
		{
			...
			{ // We need this sub-block for releasing the dc
				...
				CString lpstrText = pMenuData->GetString(m_bDrawAccelerators ? m_hAccelToDraw : nullptr);
	...
	else
	{
		...
		CString itemText = pMenuData->GetString(m_bDrawAccelerators ? m_hAccelToDraw : nullptr);
		SIZE size = {0, 0};
...
BOOL CNewMenu::ModifyODMenu(LPCTSTR lpstrText, UINT nID, int nIconNormal)
{
	...
	do
	{
		...
		ASSERT(pItemData);
		if (lpstrText && pItemData->m_szMenuText.Compare(lpstrText) != 0)
		{
			bTextChanged = TRUE;
			pItemData->SetString(lpstrText);
		}
...
CNewMenuItemData *CNewMenu::NewODMenu(UINT pos, UINT nFlags, UINT nID, LPCTSTR string)
{
	...
	else // (nFlags&MF_STRING)
	{
		...
		bModified = ModifyMenu(pos, nFlags, nID, pItemData->GetString(m_bDrawAccelerators ? m_hAccelToDraw : nullptr));
	}
	if (!bModified)
	{
...
CNewMenu *CNewMenu::FindMenuOption(LPCTSTR lpstrText, int &nLoc)
{
	...
	{
		if (m_MenuItemList[i]->m_szMenuText.Compare(lpstrText) == 0)
		{
			nLoc = i;
			return this;
		}
...
// INVALID_HANDLE_VALUE = Draw default frame's accel. nullptr = Off
...
BOOL CNewMenu::GetMenuText(UINT id, CString &string, UINT nFlags /*= MF_BYPOSITION*/)
{
	if (MF_BYPOSITION & nFlags)
	{
		...
		if (id < numMenuItems)
		{
			string = m_MenuItemList[id]->GetString(m_bDrawAccelerators ? m_hAccelToDraw : nullptr);
			return TRUE;
		}
	}
	else
...
BOOL CNewMenu::OnUnInitPopupMenu()
{
	...
	{
		pParent->m_dwOpenMenu -= 1;
		if (pParent->m_dwOpenMenu >= 0 && !pParent->m_bIsPopupMenu)
		{
...
CMenu *CNewMenu::GetSubMenu(LPCTSTR lpszSubMenuName) const
{
	...
	for (int i = 0; i < num; i++)
	{
		...
		if (name.IsEmpty())
		{
			...
			if (pItemData)
			{
				name = pItemData->GetString(m_bDrawAccelerators ? m_hAccelToDraw : nullptr);
			}
		}
...
int CNewMenu::GetMenuPosition(LPCTSTR pText)
{
	for (int i = 0; i < (int)(GetMenuItemCount()); ++i)
	{
		if (!GetSubMenu(i))
		{
			...
			{
				if (m_MenuItemList[j]->m_szMenuText.Compare(pText) == 0)
				{
					return j;
				}
			}
	...
	if (pData->m_bDoSubclass)
	{
		if (!DoDrawBorder())
		{
			if (g_Shell >= Win2000 && CNewMenu::GetAlpha() != 255)
			{
				...
				pData->m_dwData |= 4;
				SetTimer(hWnd, MENU_TIMER_ID, 50, nullptr);
			}
		}
		else
		{
...
void *CMenuTheme::SetScreenBitmap(HWND hWnd, HDC hDC)
{
	...
	CNewMenuHook::CMenuHookData *pData = CNewMenuHook::GetMenuHookData(hWnd);
	if (pData->m_Screen.m_hObject == nullptr)
	{
		// Get the desktop hDC...
		HDC hDcDsk = GetWindowDC(0);
		CDC *pDcDsk = CDC::FromHandle(hDcDsk);
...
HMODULE CNewMenuHook::m_hThemeLibrary = nullptr;
...
CNewMenuHook::CNewMenuHook()
{
	...
	{ // Detecting wine because we have some issue
		...
		//    if(Wine)
		//    {
		//      MessageBox(nullptr,_T("We are running under wine!!"),_T("wine detected"),MB_OK);
		//    }
	}
...
CMenuTheme *CNewMenuHook::RemoveTheme(DWORD dwThemeId)
{
	...
	if (pos)
	{
		...
		if (m_pRegisteredThemesList->GetCount() == 0)
		{
...
CNewMenuThreadHook::CNewMenuThreadHook()
{
	...
	m_hHookOldMenuCbtFilter = ::SetWindowsHookEx(WH_CALLWNDPROC, CNewMenuHook::NewMenuHook, nullptr, ::GetCurrentThreadId());
} // CNewMenuThreadHook
...
CNewMenuThreadHook::~CNewMenuThreadHook()
{
	...
	// clear the handle
	m_hHookOldMenuCbtFilter = nullptr;
} // ~CNewMenuThreadHook
		...
		case WM_INITDIALOG:
			if (CNewMenuHook::m_bSubclassFlag & NEW_MENU_DIALOG_SYSTEM_MENU)
			{
				...
				if (IsMenu(hMenu))
				{
					...
					if (DYNAMIC_DOWNCAST(CNewMenu, pMenu) == nullptr)
					{
...
#ifdef _TRACE_MENU_
		static HWND acthMenu = nullptr;
		static HWND acthWnd = nullptr;
	...
	if (!bSysMenu)
	{
		...
		else if ((hParentMenu = ::GetMenu(m_hWnd)) != nullptr)
		{
			    ...
			    (hParentMenu = ::GetMenu(pParent->m_hWnd)) != nullptr)
			{
				...
				{
					if (::GetSubMenu(hParentMenu, index) == pMenu->m_hMenu)
					{
	...
	for (int nPos = 0; nPos < nCount; nPos++)
	{
		...
		if (DYNAMIC_DOWNCAST(CNewMenuBar, pBar))
		{
			...
			if (pBar->m_dwStyle & CBRS_ORIENT_HORZ)
			{
				if (rect.left > 0)
				{
					pBar->SetWindowPos(nullptr, -5, rect.top, 0, 0, SWP_NOZORDER | SWP_NOSIZE | SWP_FRAMECHANGED);
				}
			}
			else
			{
				if (rect.top > 0)
				{
					pBar->SetWindowPos(nullptr, rect.left, -5, 0, 0, SWP_NOZORDER | SWP_NOSIZE | SWP_FRAMECHANGED);
				}
			}
...
BOOL ShadeRegisterClass()
{
	...
	wndcls.hIcon = (HICON) nullptr;
	wndcls.hCursor = LoadCursor(nullptr, IDC_ARROW);
	wndcls.hbrBackground = (HBRUSH) nullptr;
	wndcls.lpszMenuName = (LPCTSTR) nullptr;
...
// void SetWindowShade(HWND hWndMenu, CNewMenuHook::CMenuHookData* pData )
//{
//   if(pData==nullptr || pData->m_hRightShade!=nullptr || pData->m_hBottomShade)
//   {
//     return;
//   }
//
