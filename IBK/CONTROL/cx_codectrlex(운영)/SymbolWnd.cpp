	...
	case DLL_OUBx:
		{
			...
			CString szData = CString(exth->data, exth->size);
			...
			{
				CString	imagePath;
				struct _ejinfo* ejinfo = (struct _ejinfo*)exth->data;
			...
			{
				int jrat = 0, nrat = 0;
				struct _hjinfo* hjinfo = (struct _hjinfo*)exth->data;
	...
	case DLL_OUB:	// NOT USED. Use DLL_OUBx Only !!
		{
			...
			if (len <= L_jHeader)
				return 0;
			struct _jHeader* jHeader = (struct _jHeader*)szData.GetString();
				
			if (jHeader->jgub[0] == '5' && len >= L_ejinfo) // elw
			{
				struct _ejinfo* ejinfo = (struct _ejinfo*)szData.GetString();
				m_ejinfo = std::make_unique<Cejinfo>();
			...
			{
				int jrat = 0, nrat = 0;
				struct _hjinfo* hjinfo = (struct _hjinfo*)szData.GetString();
				m_hjinfo = std::make_unique<Chjinfo>();
...
COLORREF CSymbolWnd::GetIndexColor(int index)
{
...
CString CSymbolWnd::Parser(CString &srcstr, CString substr)
{
...
CString CSymbolWnd::Variant(int comm, CString data)
{
	CString retvalue;
...
CPen* CSymbolWnd::getAxPen(COLORREF clr, int width, int style)
{
	struct	_penR	penR {};
...
CBrush* CSymbolWnd::getAxBrush(COLORREF clr)
{
	return (CBrush*) m_pWizard->SendMessage(WM_USER, getBRUSH, (LPARAM)clr);
}

CFont* CSymbolWnd::getAxFont(CString fName, int point, int style)
{
	struct	_fontR	fontR {};
...
bool CSymbolWnd::checkMemo(CString sKey)
{
	...
	TRY
	{ 
		CFile file(dir, CFile::modeRead | CFile::typeBinary | CFile::shareDenyNone);
		UINT nBytesRead;
		do
		{
...
void CSymbolWnd::SetDataTooltip()
{
	...
	CjHeader*	jHeader = (m_hjinfo) ? (CjHeader*)m_hjinfo.get() : (CjHeader*)m_ejinfo.get();
	if (jHeader == nullptr)
		return;
