...
void CMBongWnd::Alert(struct _alertR* alertR)
{
	...
	siga = koga = jega = jgga = volume = 0.0;
	if (data[29])
	{
		dat = (char*)data[29];
...
void CMBongWnd::Palette()
{
	...
	m_clrUp2 = GetGradColor(m_clrUp, false);
	m_clrDown2 = GetGradColor(m_clrDown, false);
}
...
void CMBongWnd::calcuMinMax()
{
	...
	for (int ii = 0; ii < nSize && m_MAData.size(); ii++)
	{
...
//	COLORREF	sColor, eColor;
	...
	for ( int ii = 0 ; ii < m_nCandle && ii < nCount ; ii++ )
	{
		...
		if (pData->m_jgga > pData->m_siga)
		{
			...
			//sColor = m_clrUp2;	eColor = m_clrUp;
		}
...
void CMBongWnd::drawMA(CDC* pDC)
{
	...
	for (int ii = 0, nIndex = 0; ii < nCount && ii < (int)m_MAData.size(); ii++)
	{
...
void CMBongWnd::drawVolumn(CDC* pDC)
{
	CPoint		pt[5];
...
CRect CMBongWnd::MakeAlign()
{
	CRect	rect, rcMy, rcTip, rcParent(0, 0, 0, 0);
	const	CWnd*	pWnd = (CWnd*)GetParent();
	POINT	pt{};

	const	int	x = GetSystemMetrics(SM_CXFULLSCREEN);
	const	int	y = GetSystemMetrics(SM_CYFULLSCREEN);
...
void CMBongWnd::DrawGradRect(CDC *pDC, CRect drawRC, COLORREF sColor, COLORREF eColor, bool bRight)
{
	...
	CPen	*sPen = nullptr, *cPen = nullptr;
	...
	if (bRight)
	{
		if (!nWidth)
		{
			cPen = m_pApp->GetPen(m_pWizard, sColor);
		...
		if (!nHeight)
		{
			...
			{
				cPen = m_pApp->GetPen(m_pWizard, RGB(rs - rStep*ii, gs - gStep*ii, bs - bStep*ii));
				sPen = pDC->SelectObject(cPen);
		...
		else
		{
			for (int ii = 0; ii < nWidth; ++ii)
			{
				...
				brush = m_pApp->GetBrush(m_pWizard, RGB(rs - rStep*ii, gs - gStep*ii, bs - bStep*ii));
...
void CMBongWnd::MouseMove(CPoint pt)
{
	...
	if (GetSystemMetrics(SM_CXFULLSCREEN) - pt.x < rect.Width())
		pt.x -= rect.Width();
...
void CMBongWnd::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (!strPrice.IsEmpty())
		GetParent()->SendMessage(WM_MBONG, 0, (LPARAM)strPrice.operator LPCTSTR());
