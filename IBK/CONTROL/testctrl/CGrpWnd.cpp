...
IMPLEMENT_DYNAMIC(CGrpWnd, CWnd)
...
LONG CGrpWnd::OnUser(WPARAM wParam, LPARAM lParam)
{
	...
	{
	case DLL_OUB:
			return DispatchOutData(wParam, lParam);
		break;
...
void CGrpWnd::OnTimer(UINT_PTR nIDEvent)
{
...
int CGrpWnd::DispatchOutData(WPARAM wParam, LPARAM lParam)
{
...
{
	///i710200100000000201000000000000000000300000000001110000033023200000000112000003276820000000
	///i01000120000500010100000000000000000010000000000113003355468720000000
	  //i01000120000500010100000000000000000010000000000113003355468720000000
	CString	sParam;
	sParam.Format("%s", param);
	sParam = "/i01000120000500010100000000000000000010000000000113003355468720000000";  //test not yet
	int ilen = sParam.GetLength();
	ilen = sz_GRPINFO + sz_GRPLINE + 2;
	...
	m_ShowCnt = atoi(CString(pGrpInfo->show, sizeof(pGrpInfo->show)));   //그래프 출력갯수
	m_ReqCnt = atoi(CString(pGrpInfo->req, sizeof(pGrpInfo->req)));				//그래프 요청갯수
	m_dIndex = atoi(CString(pGrpInfo->index, sizeof(pGrpInfo->index)));     //색인 일주월분틱
	...
	int	gCnt = atoi(CString(pGrpInfo->gcnt, sizeof(pGrpInfo->gcnt)));  //그래프갯수(최대18)
	...
	for (int ii = 0; ii < gCnt; ii++)
	{
		...
		CopyMemory(GrpLine.kind, pGrpLine->kind, sizeof(pGrpLine->kind));       //그래프종류
		GrpLine.region = pGrpLine->region - ICF_CHAR;	// -ICF_CHAR				 //영역(1-9)	
		CopyMemory(GrpLine.color, pGrpLine->color, sizeof(pGrpLine->color));   //그래프색상
		GrpLine.tick = pGrpLine->tick - ICF_CHAR;	// -ICF_CHAR							 //좌우틱정보
		GrpLine.width = pGrpLine->width - ICF_CHAR;	// -ICF_CHAR					 // 그래프라인두께
		GrpLine.style = pGrpLine->style - ICF_CHAR;	// -ICF_CHAR						// 그래프라인스타일	
...
{
	for (int ii = 0; ii < m_GrpCnt; ii++)
		m_pGrpInfo[ii]->CalculateMinMax();   //test not 20230227 
}
...
void CGrpWnd::ReviseTick()
{
	...
	for (int ii = 0; ii < m_RgnCnt; ii++)
	{
		...
		for (int jj = 0, gcnt = 0; jj < 2; jj++)
		{
			...
			{
				int ival = m_pGrpInfo[kk]->GetRgnKey();
				ival = m_pGrpInfo[kk]->GetMinMaxIndex();
...
int CGrpWnd::CetBottomVerticHeight(CDC* pDC)
{
	...
	CString	sTmp;
	const	int	dCount = m_pDataInfo->GetDataCount();   //test not 20230227

	for (int ii = 0; ii < dCount; ii++)
	{
		const auto gVal = m_pDataInfo->GetGraphData(ii);  //test not 20230227
...
CString CGrpWnd::GetTipStr(CPoint point)
{
	...
	CString stmp;
	//CString dispStr = m_pGrpInfo[0]->GetDisplayPosHead(point);  //test not 20230227
	CString dispStr;
	for (int ii = 0; ii < m_GrpCnt; ii++)
	{
		//if (m_pGrpInfo[ii]->GetRgnKey() == rgnIdx)
		{
			//stmp = m_pGrpInfo[ii]->GetDisplayPosData(point);  //test not 20230227
			if (!stmp.IsEmpty())
			{
				if (!dispStr.IsEmpty())
					dispStr = dispStr + "\n" + stmp;
...
void CGrpWnd::ExcelProc()
{
	...
	CString	tmpS;
	CString	ExcelS{};
	/*CString	ExcelS = m_pGrpInfo[0]->GetExcelHead(-1);    //test not 20230227
	for (int ii = 0; ii < m_GrpCnt; ii++)
	{
