...
int COpSensCal::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_IOpSensCal = new IOpSensCal(m_COpSensCal->GetIDispatch(TRUE));
...
BOOL COpSensCal::CreateOpSensCal(CWnd* parent, CString root)
{
	...
	return CWnd::Create(NULL, "OpSensCal", WS_CHILD|WS_VISIBLE, CRect(NULL), parent, 1001);
}
...
void COpSensCal::CalcEx(LPCTSTR cur, LPCTSTR k200cur, LPCTSTR fcur, LPCTSTR time)
{
	if (m_IOpSensCal == NULL) return;
	m_IOpSensCal->CalcEx(cur, k200cur, fcur, time);
}
