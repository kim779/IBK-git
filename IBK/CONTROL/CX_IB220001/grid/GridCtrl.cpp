...
BOOL CGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle, DWORD dwExStyle)
{
	TRACE("Create\n");
	ASSERT(pParentWnd->GetSafeHwnd());

	//if (!CWnd::Create(GRIDCTRL_CLASSNAME, NULL, dwStyle, rect, pParentWnd, nID))
	if (!CWnd::CreateEx(dwExStyle, GRIDCTRL_CLASSNAME, NULL, dwStyle, rect, pParentWnd, nID))  
		return FALSE;
...
void CGridCtrl::OnDraw(CDC* pDC)
{
	...
	CRect	VisRect;
	const CCellRange VisCellRange = GetVisibleNonFixedCellRange(VisRect);
				 ...
				 BOOL bForceRepaint /* = FALSE */, BOOL bSelectCells/*=TRUE*/)
{
	...
	if (bSelectCells)
	{
		...
		{
			for (POSITION pos = m_PrevSelectedCellMap.GetStartPosition(); pos; /* nothing */)
			{
			      ...
			      BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
	...
	if (GetSingleColSelection())
		SetSelectedRange(GetFixedRowCount(), currentCell.col, GetRowCount()-1, currentCell.col, bForceRedraw, bSelectCells);
	else
			   ...
			   BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
	...
	if (GetSingleRowSelection())
		...
		SetSelectedRange(min(m_SelectionStartCell.row, currentCell.row), Left, __max(m_SelectionStartCell.row, currentCell.row), GetColumnCount()-1,
				bForceRedraw, bSelectCells);
}
			    ...
			    BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
	...
	SetSelectedRange(min(m_SelectionStartCell.row, row), min(m_SelectionStartCell.col, col), __max(m_SelectionStartCell.row, row), __max(m_SelectionStartCell.col, col),
			bForceRedraw, bSelectCells);
}
...
BOOL CGridCtrl::PasteTextToGrid(CCellID cell, COleDataObject* pDataObject, BOOL bSelectPastedCells /*=TRUE*/)
{
	...
	do
	{
		...
		while (!strLine.IsEmpty())
		{
			...
			const CCellID TargetCell(iRowVis, iColVis);
...
DROPEFFECT CGridCtrl::OnDragOver(COleDataObject* pDataObject, DWORD dwKeyState, CPoint point)
{
	...
	const	CCellID cell = GetCellFromPt(point, FALSE);
	bool	Valid = false;
...
void CGridCtrl::OnUpdateEditPaste(CCmdUI* pCmdUI)
{
	...
	const	BOOL bCanPaste = IsValid(cell) && IsCellEditable(cell) && ::IsClipboardFormatAvailable(CF_TEXT);
...
BOOL CGridCtrl::MouseOverRowResizeArea(const CPoint& point)
{
	...
	const	int	endy = start.y + GetRowHeight(idCurrentCell.row);
...
CCellRange CGridCtrl::GetVisibleNonFixedCellRange(LPRECT pRect /*=NULL*/, BOOL bForceRecalculation /*=FALSE*/)
{
	...
	for (ii = idTopLeft.row; ii < GetRowCount(); ii++)
	{
	...
	for (ii = idTopLeft.col; ii < GetColumnCount(); ii++)
	{
...
void CGridCtrl::ResetScrollBars()
{
	...
	const	CRect	VirtualRect(GetFixedColumnWidth(), GetFixedRowHeight(),	GetVirtualWidth(), GetVirtualHeight());
...
BOOL CGridCtrl::SetFixedRowCount(int nFixedRows)
{
	...
	if (!GetVirtualMode())
	{
		...
		{
			for (int ii = m_nFixedRows; ii < nFixedRows; ii++)
				for (int jj = 0; jj < GetColumnCount(); jj++)
				{
					SetItemState(ii, jj, GetItemState(ii, jj) | GVIS_FIXED | GVIS_FIXEDROW);
					SetItemBkColour(ii, jj, CLR_DEFAULT);
					SetItemFgColour(ii, jj, CLR_DEFAULT);
				}
		}
		else
		{
			for (int ii = nFixedRows; ii < m_nFixedRows; ii++)
			{
				for (int jj = 0; jj < GetFixedColumnCount(); jj++)
					SetItemState(ii, jj, GetItemState(ii, jj) & ~GVIS_FIXEDROW);

				for (int jj = GetFixedColumnCount(); jj < GetColumnCount(); jj++)
				{
					SetItemState(ii, jj, GetItemState(ii, jj) & ~(GVIS_FIXED | GVIS_FIXEDROW));
					SetItemBkColour(ii, jj, CLR_DEFAULT);
					SetItemFgColour(ii, jj, CLR_DEFAULT);
	...
	if (!GetVirtualMode())
	{
		if (nFixedCols > m_nFixedCols)
		{
			for (int ii = 0; ii < GetRowCount(); ii++)
				for (int jj = m_nFixedCols; jj < nFixedCols; jj++)
				{
					SetItemState(ii, jj, GetItemState(ii, jj) | GVIS_FIXED | GVIS_FIXEDCOL);
					SetItemBkColour(ii, jj, CLR_DEFAULT);
					SetItemFgColour(ii, jj, CLR_DEFAULT);
				}
		}
		...
		{
			for (int ii = 0; ii < GetFixedRowCount(); ii++)
				for (int jj = nFixedCols; jj < m_nFixedCols; jj++)
					SetItemState(ii, jj, GetItemState(ii, jj) & ~GVIS_FIXEDCOL);
			for (int ii = GetFixedRowCount(); ii < GetRowCount(); ii++)
				for (int jj = nFixedCols; jj < m_nFixedCols; jj++)
				{
					SetItemState(ii, jj, GetItemState(ii, jj) & ~(GVIS_FIXED | GVIS_FIXEDCOL));
					SetItemBkColour(ii, jj, CLR_DEFAULT);
...
BOOL CGridCtrl::SetRowCount(int nRows)
{
	...
	TRY
	{
		...
		if (GetVirtualMode())
		{
			...
			if (addedRows > 0)
			{
				for (int row = nRows - addedRows; row < nRows; row++)
...
int CGridCtrl::InsertColumn(LPCTSTR strHeading, UINT nFormat /* = DT_CENTER|DT_VCENTER|DT_SINGLELINE */, int nColumn  /* = -1 */)
{
	TRACE("InsertColumn\n");
...
long CGridCtrl::GetVirtualWidth() const
{
	...
	for (int ii = 0; ii < GetColumnCount(); ii++)
		lVirtualWidth += m_arColWidths[ii];
...
long CGridCtrl::GetVirtualHeight() const
{
	...
	for (int ii = 0; ii < GetRowCount(); ii++)
		lVirtualHeight += m_arRowHeights[ii];
...
BOOL CGridCtrl::IsCellVisible(int nRow, int nCol)
{
	...
	else
	{
		...
		for (int ii = TopLeft.col; ii <= nCol; ii++)
		{
			if (x >= rect.right)
				return FALSE;
			x += GetColumnWidth(ii);
	...
	else
	{
		if (nRow < TopLeft.row)
			return FALSE;
		y = GetFixedRowHeight();
		for (int ii = TopLeft.row; ii <= nRow; ii++)
		{
			if (y >= rect.bottom)
				return FALSE;
			y += GetRowHeight(ii);
...
BOOL CGridCtrl::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	...
	if (m_nRowsPerWheelNotch == -1)
	{
		...
		if (nPagesScrolled > 0)
		{
			for (int ii = 0; ii < nPagesScrolled; ii++)
			{
	...
	else
	{
		...
		if (nRowsScrolled > 0)
		{
			for (int ii = 0; ii < nRowsScrolled; ii++)
			{
...
void CGridCtrl::OnMouseMove(UINT /*nFlags*/, CPoint point)
{
	...
	if (m_bLMouseButtonDown)
	{
		...
		case MOUSE_SIZING_COL:
		{
			...
			const	CRect	oldInvertedRect(m_LastMousePoint.x, rect.top, m_LastMousePoint.x + 2, rect.bottom);
			pDC->InvertRect(&oldInvertedRect);
			const	CRect	newInvertedRect(point.x, rect.top, point.x + 2, rect.bottom);
		...
		case MOUSE_SIZING_ROW:
		{
			...
			const	CRect	oldInvertedRect(rect.left, m_LastMousePoint.y, rect.right, m_LastMousePoint.y + 2);
			pDC->InvertRect(&oldInvertedRect);
			const	CRect	newInvertedRect(rect.left, point.y, rect.right, point.y + 2);
			pDC->InvertRect(&newInvertedRect);
			ReleaseDC(pDC);
		}
		break;
...
BOOL CGridCtrl::Save(LPCTSTR filename, TCHAR chSeparator/*=_T(',')*/)
{
	...
	if (!File.Open(filename, CFile::modeWrite | CFile::modeCreate | CFile::typeText | CFile::shareDenyNone, &ex))
	{
		ex.ReportError();
		return FALSE;
	}
	...
	TRY
	{
		...
		for (int ii = 0; ii < GetRowCount(); ii++)
		{
			for (int jj = 0; jj < nNumColumns; jj++)
			{
				File.WriteString(GetItemText(ii,jj));
				File.WriteString((jj == (nNumColumns - 1)) ? _T("\n") : strSeparator);
			}
		}
...
BOOL CGridCtrl::Load(LPCTSTR filename, TCHAR chSeparator/*=_T(',')*/)
{
	...
	TCHAR	*token = nullptr, *end = nullptr;
	TCHAR	buffer[1024]{};
	CStdioFile File;
	CFileException ex;
