...
int CEdgeWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_sMapName = (LPCTSTR)m_pWizard->SendMessage(WM_USER, MAKEWPARAM(mapDLL, 1));
	...
	if (result == S_OK)
	{
		...
		webView->add_NavigationCompleted(
			Microsoft::WRL::Callback<ICoreWebView2NavigationCompletedEventHandler>(
				[this](ICoreWebView2* sender, ICoreWebView2NavigationCompletedEventArgs* args) -> HRESULT {
					AxStd::_Msg("NavigationCompleted");
					return S_OK;
				}).Get(), nullptr);
		...
		m_webView->add_FrameNavigationCompleted(Microsoft::WRL::Callback<ICoreWebView2NavigationCompletedEventHandler>(
				[this](ICoreWebView2* sender, ICoreWebView2NavigationCompletedEventArgs* args) -> HRESULT
				{
					...
					{
						TRACE(_T("WebView2 FrameNavigationCompleted - 성공\n"));
		...
		m_webView->add_NavigationStarting(
			Microsoft::WRL::Callback<ICoreWebView2NavigationStartingEventHandler>(
				[this](ICoreWebView2* sender, ICoreWebView2NavigationStartingEventArgs* args) -> HRESULT {
					...
					OutputDebugString("webview " + strUrl);
...
LRESULT CEdgeWnd::SendTR(CString strName, BYTE type, CString strData, BYTE key)
{
	...
	return m_pWizard->SendMessage(WM_USER, MAKEWPARAM(invokeTRx, strData.GetLength()), (LPARAM)buff);
}
...
BOOL CEdgeWnd::CheckCloude()
{
	...
	char chfile[500]{};
	CString stmp;
	m_sRoot += "tab\\axis.ini";

	int readL;
	memset(chfile, 0x00, 500);

	GetPrivateProfileString("CLOUDELOGIN", "USE", "0", chfile, sizeof(chfile), m_sRoot);
