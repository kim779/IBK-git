...
CBtnTab::CBtnTab()
{
	...
	m_pFont      = nullptr;
	m_pBfont     = nullptr;
	m_bitmapT_nm = nullptr;
	m_bitmapT_dn = nullptr;
	m_bitmapT_hv = nullptr;
...
//	const int tabHeight = 0;
	...
	for (int ii = 0; ii < GetItemCount(); ii++)
	{
		...
		lf.lfWeight = (bCheck ? FW_SEMIBOLD : FW_NORMAL);
		font.CreateFontIndirect(&lf);//, &dc);
		pOldFont = (CFont*)pDC->SelectObject(&font);
		
		GetItemRect(ii, &rt);
		tRc1 = rt;
		tRc1.DeflateRect(0, 0, -marginR, 0);
...
LRESULT CBtnTab::OnMouseHover(WPARAM wparam, LPARAM lparam)
{
