...
public:
	...
	int	GetRealColumnWidth(int nCol) const;	//2015.02.25 KSJ 전체화면했을때 x_rate 계산안하는거
	...
	int	InsertRow(LPCTSTR strHeading, int nRow = -1, BOOL bRefresh = TRUE);
	BOOL	InsertRow(int drag, int& drop, BOOL bRefresh = TRUE);
	...
	void	DrawHeaderNews(CDC* pDC, CRect rect);
