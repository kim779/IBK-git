...
void GUILIBDLLEXPORT UpdateMenuBarColor(HMENU hMenu = nullptr);
...
HBITMAP GUILIBDLLEXPORT LoadColorBitmap(LPCTSTR lpszResourceName, HMODULE hInst, int *pNumcol = nullptr);
...
public:
	...
	// hAccel=nullptr disable accelerator support
	virtual CString GetString(HACCEL hAccel = nullptr);
...
public:
	...
	BOOL LoadToolBar(UINT n16ToolBarID, UINT n256BitmapID, COLORREF transparentColor, HMODULE hInst = nullptr);
	...
	BOOL ModifyODMenu(UINT nPosition, UINT nFlags, UINT nIDNewItem = 0, LPCTSTR lpszNewItem = nullptr);
	...
	static COLORREF GetMenuBarColor(HMENU hMenu = nullptr);
	static COLORREF GetMenuColor(HMENU hMenu = nullptr);
...
protected:
	...
	afx_msg void OnEnterMenuLoop(BOOL bIsTrackPopupMenu)
	{
		...
		{
			DrawMenubarItem(this, GetMenu(), m_menubarItemIndex, 0);
			m_menubarItemIndex = UINT(-1);
		}
	...
	{
		const UINT nHitCode = (UINT)baseClass::OnNcHitTest(point);
#endif
		...
		if (g_Shell < Win2000 || bRemoteSession)
		{
			...
			if (!m_bInMenuLoop && nHitCode == HTMENU)
			{
				...
				if (pNewMenu)
				{
					const UINT nItemIndex = MenuItemFromPoint(m_hWnd, pNewMenu->m_hMenu, point);
					...
					if (nItemIndex != (UINT)-1)
					{
						...
						else
						{
							...
							if (CNewMenu::m_dwLastActiveItem == 0 &&
...
public:
	CNewMDIFrameWnd() : m_hShowMenu(nullptr){};
