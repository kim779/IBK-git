...
void CAxBitmap::DrawEx(CDC *pDC, RECT dest)
{
	CDC MemDC;
	CBitmap *poldbitmap;
	CRect imgrc, rc;
	...
	rc.CopyRect(&dest);
	imgrc = CRect(0, 0, m_nWidth, m_nHeight);
	imgrc.DeflateRect(2, 2);
	if (m_hObject == NULL)
		return;
		...
		rc.DeflateRect(1, 1);
		pDC->StretchBlt(rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top, &MemDC,
			imgrc.left, imgrc.top, imgrc.Width(), imgrc.Height(), SRCCOPY);
