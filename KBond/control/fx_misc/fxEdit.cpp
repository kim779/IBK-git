...
CfxEdit::CfxEdit(CFont* pFont/*NULL*/)
{
	m_pFont     = pFont;
...
BOOL CfxEdit::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
	...
	if (m_pFont != NULL) CWnd::SetFont(m_pFont);
...
{
	if (m_nID == -1)
		m_nID = GetWindowLong(m_hWnd, GWL_ID);
...
void CfxEdit::OnSetFocus(CWnd* pOldWnd) 
{
	CEdit::OnSetFocus(pOldWnd);
	SetSel(0, -1);
}

void CfxEdit::OnNcPaint()
{
