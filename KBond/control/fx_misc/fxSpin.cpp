...
CfxSpin::CfxSpin(CFont* pFont/*=NULL*/)
{
	m_pFont = pFont;
...
BEGIN_MESSAGE_MAP(CfxSpin, CStatic)
	//{{AFX_MSG_MAP(CfxSpin)
	ON_WM_ENABLE()
	ON_WM_SETFOCUS()
	ON_WM_SIZE()
...
BOOL CfxSpin::Create(DWORD attr, DWORD spType, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
	m_spType = spType;
	m_attr = attr;
	m_parent = pParentWnd;
...
void CfxSpin::Init(DWORD attr, DWORD spType, CWnd* pParentWnd)
{
	//SetFont(m_pFont);
	m_spType = spType;
	m_attr = attr;
...
void CfxSpin::SetAttribute(DWORD attr)
{
	if (attr & attrCOMMA)
		attr &= ~attrNUMERIC;
	m_attr = attr;	
...
void CfxSpin::SetSpinType(DWORD remove, DWORD add)
{
	m_spType &= ~remove;
	m_spType |= add;
	...
	if (!m_pSpin)	return;
	if (m_spType & (spFUTURE | spINDEX |spOPTION))
	{
		m_attr |= attrFLOAT;
		m_attr &= ~attrCOMMA;
		m_attr &= ~attrNUMERIC;
...
void CfxSpin::SetColors(COLORREF crBack, COLORREF crFont, bool redraw)
{
	if (m_pEdit)
	{
...
void CfxSpin::SetDecimalPlaces(int digit, bool redraw)
{
...
void CfxSpin::OnEnable(BOOL bEnable) 
{
	if(m_pEdit)
...
void CfxSpin::OnSetFocus(CWnd* pOldWnd) 
{
	if (m_pEdit)
	{
		m_pEdit->SetFocus();
...
void CfxSpin::OnSize(UINT nType, int cx, int cy) 
{
...
void CfxSpin::OnEditChange(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void CfxSpin::OnSpinChange(NMHDR* pNMHDR, LRESULT* pResult)
{
	if (!IsWindow(GetSafeHwnd()))
		return;
...
//				MessageBox("1");
				...
				lf.lfHeight = 50; strcpy(lf.lfFaceName, "굴림체");
...
void CfxSpin::SetPos(double nPos)
{
	if (m_pSpin)
		m_pSpin->SetPos(nPos);	
}

double CfxSpin::GetPos()
{
	if (!m_pSpin)
		return 0.0;
...
void CfxSpin::SetDirection(BOOL nFlag)
{
	if (nFlag)
...
void CfxSpin::SetRangeEx(double lower, double upper)
{
	if (!m_pSpin || !m_pEdit)
		return;
	if (lower == UNUSEDVAL)
...
void CfxSpin::GetRangeEx(double &lower, double &upper)
{
	if (!m_pSpin)	return;
	int low, upp;
...
void CfxSpin::SetDelta(double delta, bool redraw)
{
	if (!m_pSpin)	return;
...
double CfxSpin::GetDelta()
{
	if (!m_pSpin)	return 0.0;
	return m_pSpin->GetDelta();
}

void CfxSpin::SetText(CString text)
{
	if (m_pEdit)
		m_pEdit->SetText(text);
}

CString CfxSpin::GetText()
{
	CString text = _T("");
	if (m_pEdit)
...
void CfxSpin::SetLimitText(int len)
{
