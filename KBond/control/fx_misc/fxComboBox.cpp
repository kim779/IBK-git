...
void CfxComboBox::LoadImg(CString path)
{
	...
	{
		m_hBmpLoad = m_hBmp[BTN_XN];
	}
	//CComboBox::EnableWindow
}

void CfxComboBox::ChkFont()
{
	//COLORREF clrFont;
	//if(m_bEnable)
	//	clrFont = RGB(116,116,116);
	//else
	//	clrFont = RGB(109,109,109);
	//CGdiObject* font;
...
void CfxComboBox::OnPaint()
{
	if(m_bImgBtn)
	{
		...
		ChkFont();
		CGdiObject* font;
		font = pDC->SelectObject(m_pFont);
		if(!m_bEnable)
			pDC->SetTextColor(RGB(109, 109, 109));
...
void CfxComboBox::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	if((state & ODS_SELECTED) && (action & (ODA_SELECT | ODA_DRAWENTIRE))) {
		...
		if (m_pFont != NULL) font = pDC->SelectObject(m_pFont);
		else
		{
			if (GetParent()->GetFont())
			{
			...
			{
				font = pDC->SelectStockObject(SYSTEM_FONT);
			}
		}
