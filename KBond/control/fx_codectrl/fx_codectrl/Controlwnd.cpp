...
bool CControlwnd::ChecktypeAG(CString type, CString onlytype, CString tmpcode) //2자리로 넘어온 타입(m_arrtype)을 비교
{
	...
	else
	{
		...
		{
			int nnn = atoi(m_arrtype.GetAt(ii));
		//	nnn = atoi(type);
		//	tmps.Format(_T("저장한타입=%d 파일타입=%d \n"), atoi(m_arrtype.GetAt(ii)), atoi(type));
			
			if(atoi(m_arrtype.GetAt(ii)) == atoi(type))    //m_arrtype <-- settype 으로 받은 2자리 발행사 분류코드   ==  itype (01... 등등)
			{
		//		TRACE(tmps);
				if(m_bonlyBal)  //fx_codectrl 에 준 settype(2자리, 발행구분y,n) 중 두번째 파라미터다 
				{
					if(onlytype == _T("Y"))  
						return true;
				}
