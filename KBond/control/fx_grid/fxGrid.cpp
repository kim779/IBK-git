...
const COLORREF sdCOLOR    = PALETTERGB(255, 255, 255);  // shadow color

// updateX_20080602
static CFont	g_Font;
static CFont*	g_pFont = NULL;

// scbang : Timer 처리를 위해서 추가
const int TMI_DRAW	= 999;
const int TME_DRAW	= 50;
...
// CfxGrid
// updateX_20080602
CfxGrid::CfxGrid(LOGFONT* logfont /*NULL*/, BOOL bType /*FALSE*/)
{
	RegisterWindowClass();
...
int CfxGrid::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
{
	m_noMessage = true;
	LONG response = (LONG)SendMessage(msg, MAKEWPARAM(scode, SB_THUMBPOSITION), 0);
	m_noMessage = false;
...
{
	m_bReadyToDraw = false;	// 2011.08.22 scbang : 드로잉 로직변경
	...
	SetTimer(TMI_DRAW, 1000 / m_iDrawTimes, NULL);
}
...
void CfxGrid::OnSysColorChange() 
{
	...
	if (m_bkCOL == m_shadowCOL)	m_bkCOL = sdCOLOR;
...
void CfxGrid::OnTimer(UINT_PTR nIDEvent)
{
	// 2011.08.22 scbang : 드로잉 로직변경
	if (nIDEvent == TMI_DRAW)
	{
		KillTimer(TMI_DRAW);

//		m_bSetDrawTimer = false;
		if (m_drawRect.IsRectEmpty())
			m_bReadyToDraw = true;	// 2011.08.18 scbang : 타이머방식 변경
		else
		{
			m_drawHolding = false;
	...
	else if (nIDEvent != WM_LBUTTONDOWN)
	{
		KillTimer(nIDEvent);
		
		COLORREF  bkcol;
		int row = (int)nIDEvent / 10000;
		int col = (int)nIDEvent % 10000;

		if (!m_mapRTM.Lookup((int)nIDEvent, bkcol))
			return;

		m_sync.Lock();
		m_mapRTM.RemoveKey((int)nIDEvent);
		m_sync.Unlock();
	...
	else if (IsCellAttribute(m_idSelect, GVAT_DRAGMOVE))
	{
		...
		ClientToScreen(&clientRC); ClipCursor(clientRC);

		CDC* pDC = GetDC();
		if (pDC)
		{
			CRect invertRC;
			GetCellRect(m_idSelect.row, m_idSelect.col, invertRC);
			pDC->InvertRect(&invertRC);
			ReleaseDC(pDC);
		}
...
{    
	...
	case VK_HOME:  
	case VK_END:    
		OnKeyDown((UINT)gvitem->param, 0, 0);
		OnEditCell(m_idCurrent.row, m_idCurrent.col, (UINT)gvitem->param);
		break;
	}
...
// updateX_20080227
			if (Xscroll == 0)
			{
				for (int xcol = tlCell.col+1; xcol < m_nCols; xcol++)
				{
					if (IsCellAttribute(CIdCell(tlCell.row, xcol), GVAT_HIDDEN))
						continue;

					Xscroll = GetColumnWidth(xcol);
					if (Xscroll <= 0) continue;
...
// updateX_20080602
void CfxGrid::OnDraw(CMemDC* pDC)
{
	CRect rect;
	int row, col;
	...
	if( m_bFontDrawType == TRUE)
	{
		...
		for (row = 0; row < m_nFixedRows; row++)
		{
			...
			for (col = minVisibleCol; col <= maxVisibleCol; col++)
			{
				...
				LOGFONT *logfont = GetItemFont(row, col);
				//	strBefore.Format( "%s", logfont->lfFaceName);
				// 조금더 최적화 필요
				if( g_pFont != NULL)
				{
					LOGFONT oldlogfont;
					g_pFont->GetLogFont( &oldlogfont);
					
					if( strcmp( oldlogfont.lfFaceName, logfont->lfFaceName)
						//	strBefore.CompareNoCase( oldlogfont.lfFaceName)
						|| logfont->lfWeight != oldlogfont.lfWeight)
						//	|| logfont->lfHeight != oldlogfont.lfHeight)
					{
						g_Font.DeleteObject();
						g_Font.CreatePointFontIndirect(logfont);
						pDC->SelectObject( g_Font);
					}
					
					g_pFont = &g_Font;
				}
				else if( g_pFont == NULL)
				{
					g_Font.DeleteObject();
					g_Font.CreatePointFontIndirect(logfont);
					pDC->SelectObject( g_Font);
...
// updateX_20080602
		...
		for (row = minVisibleRow; row <= maxVisibleRow; row++)
		{
			...
			for (col = minVisibleCol; col <= maxVisibleCol; col++)
			{
				...
				if( g_pFont != NULL)
				{
					...
					//	if( strBefore.CompareNoCase( oldlogfont.lfFaceName)
					if( strcmp( oldlogfont.lfFaceName, logfont->lfFaceName)
						|| logfont->lfWeight != oldlogfont.lfWeight)
						//	|| logfont->lfHeight != oldlogfont.lfHeight)
					{
	...
	pDC->SelectStockObject(NULL_BRUSH); pDC->Rectangle(visRect);
...
BOOL CfxGrid::DrawFixedCell(CDC* pDC, int nRow, int nCol, CRect rect, BOOL bEraseBk)
{
	...
	if ((gvitem.attr & GVAT_HDRMERGE) == GVAT_HDRMERGE)
	{
		...
		int minCol = visibleCells.GetMinCol();
		int maxCol = visibleCells.GetMaxCol();
		if (minCol == nDrawCol)
		{
			for (int ii = nDrawCol; nDrawCol >= 0; ii--)
			{
				if (m_mapMerge.Lookup(ii, fixMerge))
				{
					nDrawCol = ii;
					break;
				}
		...
		m_mergesync.Lock();
		if (m_mapMerge.Lookup(nDrawCol, fixMerge))
		{
			int nLWidth, nRWidth;
			nLWidth = nRWidth = 0;
			for (int ii = 0; ii < maxMERGE; ii++)
			{
				...
				if (fixMerge.cols[ii] < minCol)	nLWidth += GetColumnWidth(fixMerge.cols[ii]);
				if (fixMerge.cols[ii] > maxCol) nRWidth += GetColumnWidth(fixMerge.cols[ii]);
				...
				if (nCol != nDrawCol)
				{
					if (minCol == fixMerge.cols[ii])
					{
						Rect.bottom = Rect.bottom - (Rect.Height() / 2);
						if (nDrawCol > m_nFixedCols) Rect.left  -= 1;
						fixMerge.rect = Rect;
					}
					else
					{
			...
			if (nLWidth > 0) fixMerge.rect.left -= nLWidth;
			if (nRWidth > 0) fixMerge.rect.right += nRWidth;

			fixMerge.rect.bottom -= 2;
			m_mapMerge.SetAt(nDrawCol, fixMerge);
		}
		m_mergesync.Unlock();
	}
	...
	LOGFONT *logfont  = GetItemFont(nRow, nDrawCol);
	CIdCell focusCell = GetFocusCell();

	if (false && (focusCell.row == nRow || focusCell.col == nDrawCol))
	{
		static LOGFONT lf;
...
// updateX_20080625
	if ((gvitem.attr & GVAT_CONTRAST) == GVAT_CONTRAST && !string.IsEmpty() && IsCellVisible(nRow, nCol))
	{
		CString	temps;
		switch (string[0])
		{
...
// updateX_20060109
	...
	if (false && m_sortColumn != -1 && m_sortColumn == nDrawCol)
	{
		if (m_ascending)
			string.Insert(0, "△");
		else
...
//	CRect	visRect; GetVisibleNonFixedCellRange(visRect);
	...
	if (gvitem.state & GVST_DROPHILITED && !pDC->IsPrinting())
	{
		...
		pDC->SelectStockObject(LTGRAY_BRUSH); //NULL_BRUSH);
// updateX_20080212
		rect.DeflateRect(1, 0, 0, 0); pDC->Rectangle(rect); rect.InflateRect(1, 0, 0, 0);
	}

// updateX_20080602
	CFont font;
	if (m_bFontDrawType == FALSE || bEraseBk == TRUE)
	{
...
// updateX_20090109
	if (true && rtmCol == -1 && m_selectLine && (gvitem.state & GVST_SELECTED) && !(gvitem.state & GVST_FOCUSED))
		pDC->SetTextColor(m_fgSELCOL);

	if ((gvitem.format & GVFM_MULTI) == GVFM_MULTI) rect.DeflateRect(0, 4);
	if (rect.top < rect.bottom && rect.left < rect.right) DrawText(pDC->m_hDC, string, -1, rect, gvitem.format);
...
// updateX_20080402
	...
	{
//		CRect clipRect;
//		if (pDC->GetClipBox(&clipRect) == ERROR || !clipRect.IntersectRect(&clipRect, &rect))
//		{
//			ReleaseDC(pDC);	return FALSE;
//		}
		...
		{
			bResult = DrawFixedCell(&m_memDC, nRow, nCol, rect, TRUE);
// updateX_20060125(manie)
			if (true || m_gridLine == GVLN_BOTH || m_gridLine == GVLN_HORZ) 
			{
				if (rect.top > 0) rect.top -= 1;
				m_memDC.MoveTo(rect.left, rect.top);    m_memDC.LineTo(rect.right+1, rect.top);
				m_memDC.MoveTo(rect.left, rect.bottom); m_memDC.LineTo(rect.right+1, rect.bottom);
			}
			if (m_gridLine == GVLN_BOTH || m_gridLine == GVLN_VERT) 
			{
				// updateX_20050530
...
//				if ((m_nFixedCols != 0 && nCol < m_nFixedCols) || (m_nFixedCols == 0 && nRow == 0 && nCol == 0))
				{
					m_memDC.MoveTo(rect.left, rect.top); m_memDC.LineTo(rect.left, rect.bottom+1);
				}
				m_memDC.MoveTo(rect.right, rect.top); m_memDC.LineTo(rect.right, rect.bottom+1);
			}
		}
		else
		{
// updateX_20060201(manie)
			...
			bResult = DrawCell(&m_memDC, nRow, nCol, rect, TRUE);
			if (nRow == maxVisibleRow || m_gridLine == GVLN_BOTH || m_gridLine == GVLN_HORZ) 
			{
				if (rect.top > 0) rect.top -= 1;
// updateX_20080218
				if (true || nRow != maxVisibleRow)
				{
					m_memDC.MoveTo(rect.left, rect.top);    m_memDC.LineTo(rect.right+1, rect.top);
				}
				m_memDC.MoveTo(rect.left, rect.bottom); m_memDC.LineTo(rect.right+1, rect.bottom);
			...
			// 2009. 6. 22. Last Line Drawing(Kim Jung-tae)
			if (visCellRange.GetMaxCol() <= nCol)
			{
				m_memDC.MoveTo(visRect.right - 1, rect.top); m_memDC.LineTo(visRect.right - 1, rect.bottom);
			}
...
// 2010. 8. 13. 첫번째 셀 깜박임현상으로 삭제(SiPark)
...
BOOL CfxGrid::IsRowResizeArea(CPoint& point) const
{
	if (m_nFixedCols > 0)
	{
...
BOOL CfxGrid::IsColResizeArea(CPoint& point) const
{
...
// updateX_20080310
		if (IsCellAttribute(CIdCell(m_nFixedRows-1, ii), GVAT_HDRMERGE))
			continue;
...
void CfxGrid::ResetScrollBars()
{
	...
	if (m_DIscrollV) sinfo.fMask |= SIF_DISABLENOSCROLL;
	...
	sinfo.fMask  = SIF_PAGE|SIF_RANGE;
	if (m_DIscrollH) sinfo.fMask |= SIF_DISABLENOSCROLL;
...
//	ResetSelectedRange();
	...
	{
		m_aryColWidth.InsertAt((INT_PTR)nColumn, (UINT)0);
		for (int row = 0; row < m_nRows; row++) 
		{
			GRID_ROW* pRow = m_aryGridRows[row];
			if (!pRow) return -1;
...
int CfxGrid::InsertRow(LPCTSTR strHeading, int nRow, int szHeight)
{
...
//	ResetSelectedRange();
	...
	{
		m_aryRowHeight.InsertAt((INT_PTR)nRow, (UINT)0);
	...
	if (strlen(strHeading))
		SetItemText(nRow, 0, strHeading);

	if (szHeight != -1)
		m_aryRowHeight[nRow] = szHeight;
	else
...
// updateX_20080402
	...
	CRect	insRect; CRangeCell range(nRow, 0, nRow, chkcol);	
	CRect	visRect; GetVisibleNonFixedCellRange(visRect);
...
CCell* CfxGrid::CreateCell(int nRow, int nCol)
{
	...
	if (nRow > 0 && nCol >= 0 && nCol < m_nCols)
	{
		...
		logfont = GetItemFont(refRow, nCol);
...
// updateX_20080402
	...
	CRect	delRect; CRangeCell range(nRow, 0, nRow, chkcol);	
	CRect	visRect; GetVisibleNonFixedCellRange(visRect);
...
BOOL CfxGrid::SetItem(const GVITEM* gvitem)
{
	...
	if (gvitem->attr & GVAT_HDRMERGE && !gvitem->misc.IsEmpty())
	{
		...
		if (!m_mapMerge.Lookup(key, fixMerge))
		{
			...
			fixMerge.bkcol = (COLORREF)gvitem->param;
...
{
	CCell* pCell = GetCell(nRow, nCol);
	//if (!pCell) return (LPARAM) -1; //121128 sspark del
	if (!pCell) return ""; //121128 sspark add
	return pCell->symbol;
}
...
BOOL CfxGrid::IsCellAttribute(CIdCell& cell, UINT attr) const
{
	CIdCell hidden = cell;
...
{  
	...
	if (IsCellAttribute(CIdCell(nRow, nCol), GVAT_HIDDEN))
		return FALSE;
// updateX_20080625
	if (GetRowHeight(nRow) <= 0 || GetColumnWidth(nCol) <= 0)
		return FALSE;
	...
	if (!(nFlags & MK_LBUTTON) || (m_mousemode == mouseNOTHING && (nFlags & MK_LBUTTON)))
	{
		...
		if (m_mousemode == mouseNOTHING && m_tooltip && m_enabletip)
		{
				...
				&& IsCellAttribute(current, GVAT_TOOLTIP))
			{
				...
				CString string = GetItemText(current.row, (int)lParam);
				if (!string.IsEmpty())
					m_tooltip.Show(textRC, string, 0, cellRC, GetItemFont(current.row, (int)lParam));
			}
		}
...
// updateX_20061124
	...
	if (nFlags & MK_LBUTTON) 
	{
		...
		case mouseMOVING:
			...
			if (current.row >= m_nFixedRows) current.row = m_nFixedRows - 1;

			GetCellRect(current.row, current.col, newRC);
			cellpt.x = newRC.left; cellpt.y = point.x;

			width = (newRC.Height() * 2) / 4;

			newRC.right = newRC.left + width;
			newRC.left  = newRC.left - (width+1);
			newRC.bottom = newRC.top + (width-1);

			oldcurr = GetCellFromPoint(m_ptMove); oldcurr.row = current.row;
			GetCellRect(oldcurr.row, oldcurr.col, oldRC);

			oldRC.right = oldRC.left + width;
			oldRC.left  = oldRC.left - (width+1);
			oldRC.bottom = oldRC.top + (width-1);

			if (m_idSelect.col != current.col)
			{
				drawIndicate(newRC);
				drawIndicate(oldRC);
			}
...
void CfxGrid::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	else if (m_mousemode != mouseOVERCOLDIVIDE && m_mousemode != mouseOVERROWDIVIDE)        
	{
		...
		if( m_bMergeRTS == FALSE)
		{
			...
			if (IsCellAttribute(m_idClick, GVAT_DRAGDROP)) // && hOldFocusWnd == GetSafeHwnd())
			{
				...
				m_timerID = (int)SetTimer(WM_LBUTTONDOWN, tmINTERVAL, 0);
			}
			else if (IsCellAttribute(m_idClick, GVAT_CHECK))
			{
				SelectRows(m_idClick);
	...
	if (m_mousemode == mouseNOTHING)
	{
		...
		{
			m_timerID = (int)SetTimer(WM_LBUTTONDOWN, tmAUTOSCRL/10, 0);
			m_timerID = (int)SetTimer(WM_LBUTTONDOWN, tmAUTOSCRL, 0);
		}
	}
	...
	case mouseMOVING:
		{
			...
			{
				CRect	rect;
				GetCellRect(m_idSelect.row, m_idSelect.col, rect);
				pDC->InvertRect(&rect);
				ReleaseDC(pDC);
			}
...
void CfxGrid::OnFixedRowClick(CIdCell& cell)
{
	...
	if (cell.col >= GetFixedColumnCount() && IsCellAttribute(cell, GVAT_DRAGMOVE))
	{
		...
		m_timerID = (int)SetTimer(WM_LBUTTONDOWN, tmINTERVALx, 0);
...
// updateX_20080212
	...
	if (nCol < m_nFixedCols) rect.DeflateRect(1, 1, 1, 1);
...
bool CfxGrid::OnComboCell(int nRow, int nCol)
{
	...
	CreateInPlaceCombo(rect, dwStyle, IDC_INPLACE_COMBO, nRow, nCol, gvitem.misc, (int)gvitem.param);
	return true;
}
...
COleDataSource* CfxGrid::CopyTextFromGrid()
{
	...
	DWORD dwLen = (DWORD)sharedFile.GetLength();
...
BOOL CfxGrid::PasteTextToGrid(CIdCell cell, COleDataObject* pDataObject)
{
	...
	CMemFile memFile((BYTE*) ::GlobalLock(hmem), (int)::GlobalSize(hmem));
	...
	memFile.Read(szBuffer, (int)::GlobalSize(hmem));
...
BOOL CfxGrid::IsValidDrop(COleDataObject*pDataObject)
{
	...
	memFile.Read(pData, (int)::GlobalSize(hmem));
	::GlobalUnlock(hmem); ::GlobalFree(hmem);
...
COLORREF CfxGrid::DisplayAttribute(int attr, CString& data)
{
	...
	int checkattr = GVAT_ZEROSUP|GVAT_ZEROSUPx|GVAT_COMMA|GVAT_FLOAT1|GVAT_FLOAT2|GVAT_FLOAT4|GVAT_CONDITION|GVAT_CONDITIONx|GVAT_DIGIT;
	...
	switch (attr & (GVAT_COMMA|GVAT_FLOAT1|GVAT_FLOAT2|GVAT_FLOAT4|GVAT_DIGIT))
	{
	case GVAT_COMMA:
		stringx = CommaModify(string);
		break;
	case GVAT_DIGIT:
		stringx.Format("%.0f", atof(string));
		break;
	case GVAT_FLOAT1:
		if (stringL >= 2)
...
CString CfxGrid::validAttribute(int row, int col, CString text)
{
	...
	if (attr & (GVAT_FLOAT1|GVAT_FLOAT2|GVAT_FLOAT4|GVAT_DIGIT)) string.Remove('.');
...
void CfxGrid::SetSelectRow(int row, bool sinit)
{
	CIdCell cell(row, 0);
...
void CfxGrid::drawIndicate(CRect rect, COLORREF color)
{
	const int cnt = 3;
	...
	brush.CreateSolidBrush(color); // bkcolor
	oldbrush = (CBrush *) pDC->SelectObject(&brush);

	if (!(drawRC.Height() % 2))
		drawRC.top++;

	point[0].y = drawRC.top;
	point[0].x = drawRC.left + (drawRC.Width()/2);

	point[1].y = drawRC.bottom;
	point[1].x = drawRC.left + cnt;

	point[2].y = drawRC.bottom;
	point[2].x = drawRC.right - cnt;

	rgn.CreatePolygonRgn(point, cnt, WINDING);
...
void CfxGrid::beginDrawHolding()
{
	m_drawHolding = true;
}

void CfxGrid::endDrawHolding()
{
	// 2011.08.22 scbang : 드로잉 로직변경
	//if (!m_bReadyToDraw)
	if (!m_bReadyToDraw || m_drawRect.IsRectEmpty())
	{
		return;
	}
...
//TRACE("CfxGrid::endDrawHolding() End %d, m_bSetDrawTimer=%d, m_bReadyToDraw=%d\n", this, m_bSetDrawTimer, m_bReadyToDraw);
}

int CfxGrid::GetDrawTimes()
{
	return m_iDrawTimes;
}

void CfxGrid::SetDrawTimes(int iDrawTimes)
{
	if (iDrawTimes < 1 || iDrawTimes > 100)
		return;
	m_iDrawTimes = iDrawTimes;
}