...
WriteLog(m_slog);
				...
				/*	TickSnapshot snap;
					int idx = GetSlotIndex("A005930");

					//if (ReadTick(idx, snap))
					{
						m_slog.Format("[RTS] ReadTick[%d]  타입=[%s] 코드=[%s] 현재가=[%s] 거래량=[%s]  대비=[%s]  등락율=[%s]  틱=[%d][%d]", 
							idx, snap.RTStype,snap.code, snap.price, snap.volume, snap.diff, snap.rate , snap.ts_ms);
						OutputDebugString(m_slog);
					}*/
